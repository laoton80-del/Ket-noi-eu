...
| Pack40P1 server-owned request provenance schema (implementation) | **READY FOR PR REVIEW** — branch `feat/pack40p1-provenance-schema-implementation`, operator phrase `APPROVE_PACK40P1_PROVENANCE_SCHEMA_IMPLEMENTATION` — additive `VionaRequestScopeKind` enum + `VionaRequest.scopeKind`/`merchantProfileId` + `MerchantProfile.vionaRequests` back relation; migration `20260714120000_pack40p1_add_viona_request_provenance` **committed, NOT applied**; structural test **22/22 PASS**; full local pack regression **24/24 PASS** (Pack36A staging QA excluded). State: **`SCHEMA_COMMITTED_MIGRATION_NOT_APPLIED`** / **`DEPLOYMENT_BLOCKED_FOR_ENVIRONMENTS_WITHOUT_MIGRATION`**. No create-path wiring, no DB access, no deploy, no backfill, no Pack40A. Evidence: `docs/product/VIONA_PACK40P1_PROVENANCE_SCHEMA_EVIDENCE.md`. Next after merge: separately authorized **`APPROVE_PACK40P3_STAGING_PROVENANCE_MIGRATION_APPLY`**. |
