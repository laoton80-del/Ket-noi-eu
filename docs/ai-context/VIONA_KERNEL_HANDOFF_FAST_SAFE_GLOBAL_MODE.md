...
| FC-P0 Local provider authority E7 staging provider activation | **READY FOR PR REVIEW (BLOCKED)** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_ACTIVATION`; baseline `f59e723…`; branch `docs/viona-fc-p0-local-provider-authority-staging-provider-activation-result`; one `POST .../activate` @ `2026-07-23T16:37:09Z` → HTTP **409**; primary `BLOCKED_E7_SEPARATE_VISIBILITY_ACTION_NOT_AUTHORIZED` (activate requires `publicB2cVisible=true` but does not set it; separate PATCH not authorized); provider remains DRAFT / visibility false; audit REGISTERED only; rows 1/1; no visibility PATCH; no retry; no Local request; no deploy/migrate; no secrets; E8–E10 unauthorized; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_ACTIVATION_RESULT.md`. |
