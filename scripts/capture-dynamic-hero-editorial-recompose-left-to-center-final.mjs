/**
 * Dynamic hero editorial recompose — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-dynamic-hero-editorial-recompose-left-to-center-final'
);
...
const VIEWPORTS = [
  ...
  { name: 'travel-390x844', route: 'travel', w: 390, h: 844 },
  ...
  { name: 'local-390x844', route: 'local', w: 390, h: 844 },
];
const ROUTES = {
...
async function main() {
  ...
  for (const vp of VIEWPORTS) {
    ...
    for (const p of cfg.paths) {
      ...
      if (await page.waitForSelector(`[data-testid="${cfg.ready}"]`, { timeout: 45000 }).catch(() => null)) break;
    }
