/**
 * Travel hero card local grammar, frame & magnet hover — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-hero-card-local-grammar-frame-and-magnet-hover'
);
...
const TRAVEL_VIEWPORTS = [
  { name: 'travel-grammar-390x844', width: 390, height: 844 },
  { name: 'travel-grammar-844x390', width: 844, height: 390 },
  { name: 'travel-grammar-768x1024', width: 768, height: 1024 },
  { name: 'travel-grammar-1024x768', width: 1024, height: 768 },
  { name: 'travel-grammar-1366x768', width: 1366, height: 768 },
  { name: 'travel-grammar-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];

const LOCAL_COMPARE = { name: 'local-compare-1366x768', width: 1366, height: 768, route: '/local' };
...
async function main() {
  ...
  try {
    ...
    await captureViewport(browser, LOCAL_COMPARE);
  } finally {
    await browser.close();
  }
}
