...
| Pack40 multi-tenant VionaRequest access enforcement (planning) | **REFINED / PLANNING ONLY** — branch `docs/pack40-tenant-scope-enforcement-planning`, initial PR #340 **MERGED** @ `95031be`; refinement commit pending sync — closes Pack34-deferred `expectedTenantId` wiring gap. **v1 correction:** rejects unsafe rule "always filter by merchant tenant when MerchantProfile exists"; adopts **dual-role** `VionaRequestPrincipalContext` + row-classification policy (consumer-classified vs registered-merchant-tenant rows) without schema change. **Implementation sliced:** Pack40A (read), Pack40B (note), Pack40C (status), Pack40D (indirect paths, separate review), Pack40S (staging QA) — each separately authorized; retired broad phrase `APPROVE_PACK40_TENANT_SCOPE_ENFORCEMENT_IMPLEMENTATION`. **No implementation authorized.** See `docs/product/VIONA_PACK40_TENANT_SCOPE_ENFORCEMENT_PLAN.md`. |
