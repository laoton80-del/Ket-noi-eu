...
| Pack40B tenant note mutation enforcement (implementation) | **DEPLOYED TO STAGING (v25)** — PR #356 @ `a165ca9` + corrective PR #357 @ `45c8f29`; Pack40BD deploy `APPROVE_PACK40BD_STAGING_NOTE_ENFORCEMENT_DEPLOY`; Serializable in-tx principal; **81/81** local tests; **not CLOSED/GREEN**; Pack40BS QA still required. Evidence: `docs/product/VIONA_PACK40B_TENANT_NOTE_ENFORCEMENT_EVIDENCE.md`, `docs/product/VIONA_PACK40BD_STAGING_NOTE_ENFORCEMENT_DEPLOYMENT_EVIDENCE.md`. |
| Pack40BD staging note enforcement deployment (execution-only) | **CLOSED / GREEN (deploy only)** — deployed master @ `45c8f29` to `viona-api-staging-eu` (**v24→v25**); pre/post health **HTTP 200**; **no migration/DB/secret/authenticated QA/note POST/production**; markers **`PACK40B_TRANSACTIONAL_NOTE_ENFORCEMENT_DEPLOYED_TO_STAGING`**, **`PACK40B_STAGING_ADVERSARIAL_QA_STILL_REQUIRED`**. Evidence: `docs/product/VIONA_PACK40BD_STAGING_NOTE_ENFORCEMENT_DEPLOYMENT_EVIDENCE.md`. |
| Pack40BS staging tenant note adversarial QA (execution-only) | **QA PASS (closure pending evidence merge)** — operator phrase `APPROVE_PACK40BS_STAGING_TENANT_NOTE_ADVERSARIAL_QA`; release **v25-verified**; PR #358 merged @ `44ff2f7`; dual-role + non-owner fixtures positively verified; consumer + merchant first notes **201**; replays **200** idempotent (**0** duplicates); non-owner successful-key reuse denied; client spoof denied; legacy + nonexistent normalized **404**; exactly **+2** `action.note` audits; **8** POSTs only; **no direct DB mutation/deploy/migration/secret/production**; Pack40A remains CLOSED/GREEN; Pack40C/D/S unimplemented; **Pack40B not yet CLOSED/GREEN** until evidence merge. Evidence: `docs/product/VIONA_PACK40BS_STAGING_TENANT_NOTE_ADVERSARIAL_QA_EVIDENCE.md`. |
