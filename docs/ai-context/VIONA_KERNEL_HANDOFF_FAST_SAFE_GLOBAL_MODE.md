...
| FC-P0 Local provider eligibility Pack A2 — read + ops control | **READY FOR PACK A2 PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_READ_AND_OPS_CONTROL`; baseline `97d9ee01f6159a7ed7d48ac3b422ddb4359bca60`; branch `feat/viona-fc-p0-local-provider-eligibility-read-ops-control`; GET `/api/local/providers` + ADMIN ops register/PATCH/activate/suspend/retire; append-only audit writes; A2 tests 30–43 PASS (DB skipped under NO_MIGRATION_APPLY); **no** client wiring / migration apply / staging activation / deploy; FC-P0 still blocked pending Pack B; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_READ_AND_OPS_CONTROL_IMPLEMENTATION_EVIDENCE.md`. |
