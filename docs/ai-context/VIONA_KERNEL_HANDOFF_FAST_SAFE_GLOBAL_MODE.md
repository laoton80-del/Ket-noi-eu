...
| Pack40A tenant principal context + read enforcement (implementation) | **LOCAL IMPLEMENTATION COMPLETE — PR REVIEW** — operator phrase `APPROVE_PACK40A_TENANT_CONTEXT_AND_READ_ENFORCEMENT` — server-owned `VionaRequestReadPrincipalContext`; provenance-aware read where builder; direct list/detail only via opt-in `directReadPolicy: 'pack40a_provenance'`; consumer (`scopeKind=consumer`, `merchantProfileId=null`), merchant (exact profile ID + tenant snapshot), `legacyUnresolved` fail-closed; inactive merchant may read historical merchant rows; dual-role consumer+merchant branches coexist; **39/39** Pack40A tests + full local pack regression PASS; **no schema/migration/DB/staging/deploy**; note/status/indirect paths unchanged; Pack40B/C/D/S separately authorized. Evidence: `docs/product/VIONA_PACK40A_TENANT_CONTEXT_READ_ENFORCEMENT_EVIDENCE.md`. **Staging deploy/QA requires separate authorization.** |
