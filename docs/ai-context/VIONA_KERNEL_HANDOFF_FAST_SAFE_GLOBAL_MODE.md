...
### DEFERRED / BACKLOG (recorded, not authorized — logged during Pack37 implementation sync)

Explicitly logged here per operator instruction so neither item is lost between sessions. Neither
is authorized by this log entry — both require their own future, separately-authorized pack.
