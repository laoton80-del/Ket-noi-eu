...
| Pack40P4W staging merchant backfill write (execution-only) | **READY FOR PR REVIEW** — branch `chore/pack40p4-staging-merchant-backfill-write`, operator phrase `APPROVE_PACK40P4_STAGING_MERCHANT_BACKFILL_WRITE` — exact **5** rows updated on staging (`legacyUnresolved→merchant` + FK only); post-state **5 merchant / 5 legacy / 0 consumer**; digest revalidated; **no consumer backfill, deploy, migration, secret, production, Pack40A**. Evidence: `docs/product/VIONA_PACK40P4_STAGING_MERCHANT_BACKFILL_WRITE_EVIDENCE.md`. **Pack40P5 still required**. |
