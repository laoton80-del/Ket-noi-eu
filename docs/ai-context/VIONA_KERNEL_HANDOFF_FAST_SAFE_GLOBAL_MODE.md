...
| Pack40P2 create-path provenance wiring (implementation) | **READY FOR PR REVIEW** — branch `feat/pack40p2-create-path-provenance-wiring`, operator phrase `APPROVE_PACK40P2_CREATE_PATH_PROVENANCE_WIRING` — Pack19 assigns `scopeKind=consumer`/`merchantProfileId=null`; Pack35 assigns `scopeKind=merchant` + trusted `merchantProfileId`; client provenance keys forbidden on Pack19 body; **14/14** P2 tests + **25/25** full local pack regression PASS. **No DB, deploy, backfill, Pack40A.** Evidence: `docs/product/VIONA_PACK40P2_CREATE_PATH_PROVENANCE_EVIDENCE.md`. P2D deploy **not authorized**. Next after merge: **`APPROVE_PACK40P2D_STAGING_CREATE_PATH_PROVENANCE_DEPLOY`**. |
