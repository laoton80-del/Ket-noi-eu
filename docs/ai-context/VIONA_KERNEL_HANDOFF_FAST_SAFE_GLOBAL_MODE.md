...
| Modern Home Fashion-Tech mobile activation | **PLAN ONLY / NOT IMPLEMENTED** — recovery diagnosis `MODERN_MOBILE_UI_ALREADY_ON_MASTER_BUT_NOT_ACTIVE` (Fashion-Tech Home wired on master but gated to **web + width ≥ 769 + B2C Home**; native never enters); historical worktree recovery **not** required; architecture `MOBILE_WEB_FIRST_THEN_NATIVE_ADAPTATION`; do **not** flip `FASHION_HOME_DESKTOP_MIN_WIDTH` or remove web-only check without chrome contract; implementation **NOT AUTHORIZED**; Wave 2 physical native **NOT RUN**; Pack40DR preserved; Pack40S **NOT AUTHORIZED**. Plan: `docs/product/VIONA_MODERN_HOME_FASHION_SHELL_MOBILE_ACTIVATION_PLAN.md`. |
