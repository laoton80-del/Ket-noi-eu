/**
 * VIONA.UI.TRAVEL.HERO_DEPTH_RESTORE_WITH_EDITORIAL_TITLE_BALANCE.PACK_4
 */
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    const heroR = hero ? r(hero) : null;
    const layerR = layer ? r(layer) : null;
    const qhR = quickHelp ? r(quickHelp) : null;
    const titleW = title ? Math.round(r(title).width) : null;
    const heroH = heroR ? Math.round(heroR.height) : null;
    const heroW = heroR ? Math.round(heroR.width) : null;
    return {
      heroHeightPx: heroH,
      heroWidthPx: heroW,
      quickHelpTopPx: qhR ? Math.round(qhR.top) : null,
      quickHelpVisible: qhR ? qhR.top < window.innerHeight && qhR.height > 0 : null,
      textLayerTopPx: layerR ? Math.round(layerR.top) : null,
      textLayerHeightPx: layerR ? Math.round(layerR.height) : null,
...
async function main() {
  ...
  const all = { capturedAt: new Date().toISOString(), pack: 'hero-depth-restore-pack-4' };
  ...
  const shots = [
    ...
    { name: '1366-fullscreen-transport', w: 1366, h: 768, fs: true, ctx: 'taxi' },
  ...
  for (const shot of shots) {
    ...
    await page.locator('body').screenshot({ path: path.join(OUT, `${shot.name}.png`) });
    console.log(
      `${shot.name}: heroH=${all[shot.name].heroHeightPx}px title=${all[shot.name].titleFontSizePx}px qhTop=${all[shot.name].quickHelpTopPx}`
    );
