...
| FC-P0 Local provider authority E3 staging API deploy | **READY FOR PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_API_DEPLOY`; baseline/source `de59110a6c9a1bfef7fc0d7c68be24513ffe9781`; branch `docs/viona-fc-p0-local-provider-authority-staging-api-deploy-result`; one `fly deploy --app viona-api-staging-eu --remote-only` @ `2026-07-23T10:45:07Z`–`10:48:14Z` exit 0 → Fly **v29** / image `deployment-01KY798FWDYE8YM0ZD4QW98JP0` / digest `sha256:b4c42e5c…`; pre-deploy rollback target exact **v28**; `/health` 200; unauth Local routes **401**; Pack A1 still applied/clean; rows 0/0; no DB release hook; no secret/config mutation; no provider/Business/User mutation; no authenticated live QA; no client deploy; rollback not required; secondary canonical §9 `READY_FOR_CONTROLLED_PROVIDER_REGISTRATION`; **E4–E10 not authorized**; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_API_DEPLOY_RESULT.md`. |
