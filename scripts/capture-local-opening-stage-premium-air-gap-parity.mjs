/**
 * Local opening stage premium air gap parity — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-local-opening-stage-premium-air-gap-parity'
);
...
const LOCAL_VIEWPORTS = [
  { name: 'local-air-gap-390x844', width: 390, height: 844 },
  { name: 'local-air-gap-844x390', width: 844, height: 390 },
  { name: 'local-air-gap-768x1024', width: 768, height: 1024 },
  { name: 'local-air-gap-1024x768', width: 1024, height: 768 },
  { name: 'local-air-gap-1366x768', width: 1366, height: 768 },
  { name: 'local-air-gap-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];

const TRAVEL_COMPARE = { name: 'travel-compare-1366x768', width: 1366, height: 768, route: '/travel' };
...
async function captureViewport(browser, vp) {
  ...
  const routes = vp.route ? [vp.route, '/tabs/travel', '/TravelHub'] : ['/local', '/tabs/local', '/LocalHub'];
...
async function main() {
  ...
  try {
    for (const vp of LOCAL_VIEWPORTS) {
      await captureViewport(browser, vp);
    }
    await captureViewport(browser, TRAVEL_COMPARE);
  } finally {
    await browser.close();
  }
}
