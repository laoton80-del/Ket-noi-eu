...
**Prior sessions:** Sessions 1–4 **PASS**; native secret-tap/PIN **NOT COMPLETED**
**Run master baseline:** `bcad94b`
...
| **Pause triggered** | **No** |
| **Overall controlled pilot session 5** | **PASS** (staging / public HTTPS — user, merchant, ops read-only API) |

---

## Session verdict (prep — superseded by run above)

| Layer | Carried forward (sessions 1–4) | Session 5 execution |
|-------|-------------------------------|---------------------|
...
| **Pause triggered** | **No** (sessions 1–4) | **No** |
| **Overall session 5** | — | **PASS** (API) |
...
| Goal | Detail |
|------|--------|
| **What** | Continue controlled **no-charge** Local pilot on **public HTTPS** staging — same scope as sessions 1–4 per readiness review Option A |
| **Why session 5** | Fresh operator evidence after post–sessions 1–4 readiness review; repeat API gates + Ops Audit corroboration |
...
## 3. Preconditions (start session 5 run only if all PASS)

| # | Criterion | Carried forward | Session 5 check |
|---|-----------|-----------------|-----------------|
| 1 | `master` / `origin` ≥ `8601354` | **PASS** | Record hash at session start |
| 2 | Readiness review Option A acknowledged | **PASS** @ `8601354` | Operator read review + this doc |
...
| 5 | Public HTTPS smoke exit 0 | **PASS** @ session 4 | `node scripts/smoke-public-staging-api.mjs https://viona-api-staging-eu.fly.dev` |
...
| 10 | Native secret-tap | **NOT COMPLETED** | Not a blocker for API/web session 5 |

---

## 4. Proven gates (carry forward — re-verify each session 5 run)

Re-run public HTTPS smoke and record PASS/FAIL. Any **FAIL** triggers §6 pause.

| Gate | Requirement | Prior reference |
|------|-------------|-----------------|
| **Health** | `GET /health` HTTP 200 | Session 4 §11 |
...
| **Create** | User A creates Local request HTTP 201 | Session 4 §11 |
| **Merchant inbox** | Merchant M inbox visible | Session 4 §11 |
| **Confirm** | Merchant M confirm HTTP 200 | Session 4 §11 |
| **Decline** | Merchant M decline HTTP 200 | Session 4 §11 |
| **Tenant — user** | User B isolation | Session 4 §11 |
| **Tenant — merchant** | Merchant N isolation | Session 4 §11 |
| **Ops admin login** | `Role.ADMIN`; no dev JWT | Session 2 §9 |
| **Ops audit list** | `GET /api/local/ops/requests` HTTP 200 | Session 4 §11.4 |
| **Ops audit detail** | `GET /api/local/ops/requests/:id` HTTP 200 | Session 4 §11.4 |
| **Non-admin ops denied** | Unauthed 401; B2C/merchant 403 | Session 4 §11.4 |
...
| **No-charge safety** | `walletMode` `REQUEST_ONLY_NO_CHARGE`; `walletPhase` `NONE`; `paymentCaptured` false | Session 4 §11.3 |
...
**Pause session 5** and follow playbook §8 if **any** occur:
...
## 7. Session 5 evidence table (`CONTROLLED_PILOT_SESSION_5.RUN.1`) — 2026-05-24

| Field | Value |
|-------|--------|
| **Date / timestamp (UTC)** | 2026-05-24 (UTC) |
| **Operator** | automated smoke (no initials) |
| **master / origin @ start** | `bcad94b` |
...
- Confirm target: `9c12fcd1-6a4d-4bd7-993c-19d638ed2a7c`
- Decline target: `d19514c3-de83-41b5-9fe6-4ba1f7f15359`
- Ops detail opened: `d19514c3-de83-41b5-9fe6-4ba1f7f15359` (declined row)

---

## 7b. Session 5 evidence table (template — superseded by §7)
...
| **Expo web ops UI** | **PASS** @ session 2 §5 — valid for API/web session 5; not a substitute for native §8 checklist |
...
## 9. Explicit limitations (session 5)
...
## 11. User / merchant + ops visibility (`CONTROLLED_PILOT_SESSION_5.RUN.1`) — 2026-05-24
...
## 12. Follow-up after session 5 run

1. Optional: complete §8 native attestation on stable device — only then update native verdict to PASS.
2. **Session 6+** or sessions 1–5 rollup extension — same Option A constraints unless pause criteria fire.
