/**
 * VIONA.UI.TRAVEL.LOCAL_ASSISTANCE.PREMIUM_CONCIERGE_RECOMPOSE.PACK_1
 */
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      subtitlePresent:
        card?.textContent?.includes('Tìm dịch vụ, địa điểm và local guides quanh nơi sở tại.') ?? false,
      searchTitlePresent: card?.textContent?.includes('Tìm địa điểm cần đến') ?? false,
      searchNotePresent:
        card?.textContent?.includes('Gợi ý nơi gần bạn theo điểm đến — chưa tự đặt dịch vụ.') ?? false,
    };
  });
}
...
async function main() {
  ...
  for (const vp of [
    ...
    console.log(
      `${vp.name}: card=${all[vp.name].cardHeightPx}px search=${all[vp.name].searchHeightPx}px primary=${all[vp.name].primaryCtaHeightPx}px secondary=${all[vp.name].secondaryCtaHeightPx}px`
    );
    await ctx.close();
  }
