...
| Pack40DRD staging operator recovery deployment | **DEPLOYED TO STAGING (no live QA)** — operator phrase `APPROVE_PACK40DRD_STAGING_RECOVERY_DEPLOY`; deployed master @ `a84f46d` to `viona-api-staging-eu` via `fly deploy --remote-only`; release **v27→v28**; image `deployment-01KXN3M9E6NWTFAE5YMW60T9FH`; pre/post `/health` **HTTP 200**; smoke checks passed; **no** recovery POST, provider lookup/send, escrow action, DB/migration/secret/production; markers `PACK40DR_OPERATOR_RECOVERY_DEPLOYED_TO_STAGING` + `PACK40DR_LIVE_RECOVERY_QA_STILL_REQUIRED`; recovery **not** CLOSED/GREEN; initial controlled Pack40D CLOSED/GREEN preserved. Evidence: `docs/product/VIONA_PACK40DRD_STAGING_RECOVERY_DEPLOYMENT_EVIDENCE.md`. Next: separately authorize Pack40DRS staging recovery QA. |
