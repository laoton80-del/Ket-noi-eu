...
| Pack40A tenant principal context + read enforcement (implementation) | **CLOSED / GREEN (local)** — PR #352 **MERGED** @ `ef6172e`, operator phrase `APPROVE_PACK40A_TENANT_CONTEXT_AND_READ_ENFORCEMENT` — server-owned `VionaRequestReadPrincipalContext`; provenance-aware read where builder; direct list/detail only via opt-in `directReadPolicy: 'pack40a_provenance'`; consumer (`scopeKind=consumer`, `merchantProfileId=null`), merchant (exact profile ID + tenant snapshot), `legacyUnresolved` fail-closed; **39/39** Pack40A tests + full local pack regression PASS; **no schema/migration/DB**; note/status/indirect paths unchanged. Evidence: `docs/product/VIONA_PACK40A_TENANT_CONTEXT_READ_ENFORCEMENT_EVIDENCE.md`. |
| Pack40AD staging read enforcement deployment (execution-only) | **CLOSED / GREEN (deploy only)** — PR #353 **MERGED** @ `3d96f83`, operator phrase `APPROVE_PACK40AD_STAGING_READ_ENFORCEMENT_DEPLOY` — deployed merged master @ `ef6172e` to `viona-api-staging-eu` (**v23→v24**); pre/post health **HTTP 200**; **no migration/DB/secret/authenticated QA/production**; markers **`PACK40A_READ_ENFORCEMENT_DEPLOYED_TO_STAGING`**. Evidence: `docs/product/VIONA_PACK40AD_STAGING_READ_ENFORCEMENT_DEPLOYMENT_EVIDENCE.md`. |
| Pack40AS staging tenant adversarial read QA (execution-only) | **QA GREEN — evidence PR review** — operator phrase `APPROVE_PACK40AS_STAGING_TENANT_ADVERSARIAL_QA` — release **v24-verified**; positive dual-role fixture; consumer + merchant + historical authorized; legacy unresolved fail-closed; non-owner denied; client spoof attempts denied; existence-leak-safe 404 normalization; **0** data mutation; **24+3** bounded GETs; inactive/malformed live fixtures not manufactured (local tests only); result **`PACK40A_STAGING_ADVERSARIAL_QA_GREEN`**; Pack40A **CLOSED/GREEN pending evidence merge**; Pack40B/C/D/S separately authorized. Evidence: `docs/product/VIONA_PACK40AS_STAGING_TENANT_ADVERSARIAL_QA_EVIDENCE.md`. |
