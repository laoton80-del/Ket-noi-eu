...
| Mobile profile/language contextualization Phase 2 | **IMPLEMENTATION COMPLETE ON BRANCH / READY FOR PR REVIEW — NOT CLOSED ON MASTER** — branch `feat/viona-mobile-profile-language-contextualization-phase-2`; operator phrase `APPROVE_VIONA_MOBILE_PROFILE_LANGUAGE_CONTEXTUALIZATION_PHASE_2`; removes legacy absolute floating ProfileSwitcher / SmartTrioLanguageChip presentation; mounts `VionaShellAccountLanguageActions` in bottom-tab + left-rail chrome (same exact-one gate as SOS); fashion / Local / Travel / Academy keep contextual hosts; SOS Phase-1 + left-rail regression green; browser QA 390/430/768/1024/1366 green; Wave 2 physical native **NOT RUN**; Pack40DR wait-state preserved; Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_MOBILE_PROFILE_LANGUAGE_CONTEXTUALIZATION_PHASE_2_EVIDENCE.md`. |
