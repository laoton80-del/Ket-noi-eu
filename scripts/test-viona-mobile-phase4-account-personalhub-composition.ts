...
assert('P4-C not started', !existsSync(path.join(root, 'scripts/test-viona-mobile-phase4-account-responsive-refinement.ts')));
...
assert(
  ...
  changed.length > 0 &&
    changed.every((p) => P4B2_PRIMARY_PATHS.has(p) || P4B2_CONDITIONAL_LINEAGE_PATHS.has(p)) &&
    changed.length <= 10
);
assert(
