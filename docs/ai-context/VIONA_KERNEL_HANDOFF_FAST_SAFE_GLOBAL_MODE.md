...
| FC-P0 Local provider eligibility Pack A1 — schema + domain enforcement | **READY FOR PACK A1 PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_SCHEMA_DOMAIN_ENFORCEMENT`; baseline `4d0e6cff083f86ef153979446be4785bdbcfb73b`; branch `feat/viona-fc-p0-local-provider-eligibility-schema-domain-enforcement`; structure-only migration `20260722120000_add_local_provider_eligibility_authority` (zero rows / no apply); domain helpers + create enforcement inside `$transaction`; A1 tests 1–29 PASS; **no** A2 routes/client wiring/provider activation/deploy; FC-P0 still blocked pending A2+B; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_SCHEMA_DOMAIN_ENFORCEMENT_IMPLEMENTATION_EVIDENCE.md`. |
