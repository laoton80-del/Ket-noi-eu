...
| FC-P0 Local create business-context + submit-guard remediation | **READY FOR PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_CREATE_BUSINESS_CONTEXT_AND_SUBMIT_GUARD_REMEDIATION`; baseline `028187c`; source-backed provider options via existing `GET /api/tourism/discover` (zero-history); raw UUID consumer field removed; `runLocalCreateSubmit` locks `inFlight` before JWT await; executed DI behavioral tests; no backend/schema/payment/deploy; `REQUEST_ONLY_NO_CHARGE` preserved; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred. Evidence: `docs/product/VIONA_FC_P0_LOCAL_CREATE_BUSINESS_CONTEXT_AND_SUBMIT_GUARD_REMEDIATION_EVIDENCE.md`. |
