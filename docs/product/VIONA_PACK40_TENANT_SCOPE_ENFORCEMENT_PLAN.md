...
| **Pack40D** | Indirect paths after per-path review | **Initial controlled merchant execution CLOSED/GREEN** (PR #375+#376; staging **v28**; sole trigger `internalAuthenticatedController`); signed-webhook DISABLED; `approvedInternalDispatch` UNWIRED; consumer/legacy UNSUPPORTED; **Pack40DR audit #377**; **Pack40DR1 schema APPLIED to staging**; **Pack40DR2/DR3A/DR3B merged**; **operator recovery DEPLOYED to staging v28**; **Pack40DRS0 endpoint safety QA PASS** (4 POSTs: unauth 401, non-admin 403, nonexistent 404, completed already_terminal; all deltas 0); non-terminal stranded recovery QA still required; recovery **not** CLOSED/GREEN; no scheduler/worker; other Pack40D triggers/providers **not** closed |
