...
| FC-P0 Local provider authority plan lifecycle/write/audit/consistency remediation | **MERGED ON MASTER** — PR #416 squash `@ 435ddcd0f59b6e9295755398a54482788d7948ed` (reviewed head `e83bc6727ad59a3d2d18396ea635ee81a5574eb4`); lifecycle/write routes locked; strict review **blocked** on audit completeness / PATCH / race / tests / A1 card. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_LIFECYCLE_WRITE_AUDIT_CONSISTENCY_REMEDIATION.md`. |
| FC-P0 Local provider authority audit-model + implementation-readiness remediation | **READY FOR FINAL PLAN REMEDIATION PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_AUDIT_MODEL_AND_IMPLEMENTATION_READINESS_REMEDIATION`; baseline `435ddcd0f59b6e9295755398a54482788d7948ed`; locks complete audit Prisma model + dedicated enums + `ROLE_ADMIN` actor; removes eligibility `suspensionReason`; no-change PATCH; READ COMMITTED race A–E; post-activation empty-name policy; in-repo tests 1–43; executable A1 card; Pack A1 phrase **UNAUTHORIZED**; docs-only; FC-P0 still blocked; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_AUDIT_MODEL_AND_IMPLEMENTATION_READINESS_REMEDIATION.md`. |
