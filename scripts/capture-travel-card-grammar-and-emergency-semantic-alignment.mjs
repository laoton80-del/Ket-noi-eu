/**
 * Travel card grammar + emergency SOS semantic QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-card-grammar-and-emergency-semantic-alignment'
);
...
const VIEWPORTS = [
  { name: 'travel-card-grammar-1366x768', width: 1366, height: 768 },
  { name: 'travel-card-grammar-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
  { name: 'travel-card-grammar-1024x768', width: 1024, height: 768 },
  { name: 'travel-card-grammar-390x844', width: 390, height: 844 },
];
...
async function waitForTravelCards(page) {
  await page.waitForSelector('[data-testid="travel-flagship-emergency"]', { timeout: 60_000 });
  await page.waitForSelector('[data-testid="travel-flagship-cards-row"]', { timeout: 15_000 });
}
        ...
        () =>
          Boolean(
            document.body.textContent?.includes('Travel') ||
              document.querySelector('[data-testid="travel-flagship-emergency"]')
          ),
        { timeout: 45_000 }
      )
      .then(() => true)
