/**
 * Travel FX premium glass strip pack 2 — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-fx-premium-glass-strip-pack-2'
);
...
const VIEWPORTS = [
  { name: 'fx-glass-390x844', width: 390, height: 844 },
  { name: 'fx-glass-768x1024', width: 768, height: 1024 },
  { name: 'fx-glass-1024x768', width: 1024, height: 768 },
  { name: 'fx-glass-1366x768', width: 1366, height: 768 },
  { name: 'fx-glass-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function openTravelRoute(page) {
  ...
  throw new Error('Travel FX premium glass strip readiness failed.');
}
...
async function measureFxStrip(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      fxBelowWeather: fxRect && weatherRect ? fxRect.top >= weatherRect.top : null,
    };
  });
}
...
async function measureMobileOverlap(page) {
  return page.evaluate(({ floatReserve }) => {
    ...
    const fx = document.querySelector('[data-testid="travel-destination-context-fx-row"]')?.getBoundingClientRect();
    ...
    return {
      viewport: vh,
      floatReserve,
      fxBottom: fx ? Math.round(fx.bottom) : null,
      fxClipped: fx ? fx.bottom > vh || fx.top < 0 : null,
      chips: results,
...
async function main() {
  ...
  try {
    for (const vp of VIEWPORTS) {
      ...
      const metrics = await measureFxStrip(page);
