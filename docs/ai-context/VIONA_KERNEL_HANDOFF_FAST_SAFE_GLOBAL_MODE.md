...
| FC-P0 Local provider authority E8 Case B Vercel CLI tooling-preparation decision | **DOCS BRANCH — UNCOMMITTED PENDING PR** — branch `docs/viona-fc-p0-local-provider-authority-e8-case-b-vercel-cli-tooling-preparation-decision`; baseline `5f658cd…`; decision `VERCEL_CLI_PINNED_GLOBAL_INSTALLATION_RECOMMENDED_FOR_SEPARATE_AUTHORIZATION` (pin `vercel@56.5.0`); **no** install/fetch/login/project/link/deploy; install phrase PROPOSED only; B1A retry unauthorized. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_VERCEL_CLI_TOOLING_PREPARATION_DECISION.md`. |
