/**
 * Pack 35 — Travel language switch to Vietnamese QA.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-language-switch-to-vi-fix-pack-35'
);
...
async function openTravel(page) {
  await page.goto(`${BASE}/travel?pack35=${BUST}`, {
...
async function clickLanguageAndPickVietnamese(page) {
  ...
  if (await langLabel.first().isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    const globeRow = page.locator('div').filter({ has: page.locator('[class*="utilBtn"]') }).first();
    await page.locator('text=Language').first().click({ timeout: 10_000 }).catch(async () => {
      await page.evaluate(() => {
...
async function applyVietnameseViaStorage(page) {
...
const proof = { pack: 35, captures: {}, refresh: {} };
...
proof.captures.enDefault = proofMatches(EN_MARKERS, await collectProof(page1366));
await page1366.screenshot({
  path: path.join(OUT_DIR, 'travel-1366x768-english-default.png'),
  fullPage: true,
});
...
try {
  ...
  await applyVietnameseViaStorage(page1366);
}
proof.languageSwitchPath = switchPath;
proof.captures.afterSwitch = proofMatches(VI_MARKERS, await collectProof(page1366));
await page1366.screenshot({
  path: path.join(OUT_DIR, 'travel-1366x768-vietnamese-after-switch.png'),
  fullPage: true,
});

const qhBox = await page1366.locator('[data-testid="travel-flagship-cards-row"]').boundingBox();
const sitBox = await page1366.locator('[data-testid="travel-situations-section"]').boundingBox();
const vw = 1366;
if (qhBox && sitBox) {
  await page1366.screenshot({
    path: path.join(OUT_DIR, 'travel-1366x768-vietnamese-quickhelp-situations.png'),
    clip: {
      x: 0,
      y: Math.max(0, qhBox.y - 12),
      width: vw,
      height: sitBox.y + sitBox.height - qhBox.y + 24,
    },
  });
}
...
proof.refresh.afterReload = proofMatches(VI_MARKERS, await collectProof(page1366));
...
console.log(`Pack 35 evidence → ${OUT_DIR}`);
