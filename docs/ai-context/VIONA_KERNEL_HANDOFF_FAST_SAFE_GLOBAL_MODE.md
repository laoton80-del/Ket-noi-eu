...
| FC-P0 Local provider authority staging provider activation retry | **READY FOR PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_ACTIVATION_RETRY_AFTER_PUBLIC_B2C_VISIBILITY_ENABLEMENT`; baseline `d39a1a1…`; branch `docs/viona-fc-p0-local-provider-authority-staging-provider-activation-retry-result`; login x1 → ADMIN; one `POST .../activate` empty body @ `2026-07-23T18:50:15Z` → HTTP **200**; conclusion `CONTROLLED_STAGING_PROVIDER_ACTIVATED_AFTER_PUBLIC_B2C_VISIBILITY_ENABLEMENT`; ACTIVE / visibility true / `GENERIC_REQUEST`; audit REGISTERED→CONFIG_UPDATED→ACTIVATED; rows 1/3; PATCH **0**; E8–E10 unauthorized; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_ACTIVATION_RETRY_RESULT.md`. |
