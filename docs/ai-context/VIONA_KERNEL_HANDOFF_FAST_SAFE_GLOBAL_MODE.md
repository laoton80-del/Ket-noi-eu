...
| FC-P0 Local provider authority staging public B2C visibility enablement | **READY FOR PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_PUBLIC_B2C_VISIBILITY_ENABLEMENT`; baseline `b2e84f8…`; branch `docs/viona-fc-p0-local-provider-authority-staging-provider-public-b2c-visibility-enablement-result`; prior login HTTP **401** preserved (zero mutation); fresh login x1 → ADMIN; one `PATCH .../providers/:businessId` `{publicB2cVisible:true}` @ `2026-07-23T17:10:15Z` → HTTP **200**; conclusion `CONTROLLED_PROVIDER_PUBLIC_B2C_VISIBILITY_ENABLED_WHILE_REMAINING_DRAFT`; audit REGISTERED→CONFIG_UPDATED; rows 1/2; activation **0**; activation retry / E8–E10 unauthorized; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_PUBLIC_B2C_VISIBILITY_ENABLEMENT_RESULT.md`. |
