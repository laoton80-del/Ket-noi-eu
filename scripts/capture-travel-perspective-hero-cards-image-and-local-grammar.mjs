/**
 * Travel perspective hero cards — image + Local grammar QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-perspective-hero-cards-image-and-local-grammar'
);
...
const VIEWPORTS = [
  { name: 'travel-perspective-grammar-390x844', width: 390, height: 844 },
  { name: 'travel-perspective-grammar-844x390', width: 844, height: 390 },
  { name: 'travel-perspective-grammar-768x1024', width: 768, height: 1024 },
  { name: 'travel-perspective-grammar-1024x768', width: 1024, height: 768 },
  { name: 'travel-perspective-grammar-1366x768', width: 1366, height: 768 },
  { name: 'travel-perspective-grammar-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function expandPerspectiveSection(page) {
  const row = page.locator('[data-testid="travel-perspective-cards-row"]');
  await row.scrollIntoViewIfNeeded();
  await row.waitFor({ state: 'visible', timeout: 30_000 });
  const card = page.locator('[data-testid="travel-perspective-vietnameseAbroad"]').first();
...
async function main() {
  ...
  try {
    for (const vp of VIEWPORTS) {
      ...
      await expandPerspectiveSection(page);
      await page.waitForTimeout(vp.fullscreen ? 900 : 700);
