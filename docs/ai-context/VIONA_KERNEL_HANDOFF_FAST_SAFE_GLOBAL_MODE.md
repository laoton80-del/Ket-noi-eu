...
| Modern Home Fashion-Tech mobile activation | **PLAN MERGED ON MASTER (PR #396 @ `a51a633`) / Phase A resolver foundation in progress or on branch — NOT visually activated** — recovery diagnosis `MODERN_MOBILE_UI_ALREADY_ON_MASTER_BUT_NOT_ACTIVE`; architecture `MOBILE_WEB_FIRST_THEN_NATIVE_ADAPTATION`; Phase A introduces `resolveFashionHomeShellMode` with compatibility `isFashionHomeDesktopShell` ≡ `mode === 'desktop'`; **zero visual activation** (769/web/native behavior unchanged); mobile/tablet modes are metadata only; PR #395 preserved; SOS Phase 1 preserved; Wave 2 physical native **NOT RUN**; Phase B **NOT AUTHORIZED**; Pack40DR preserved; Pack40S **NOT AUTHORIZED**. Plan: `docs/product/VIONA_MODERN_HOME_FASHION_SHELL_MOBILE_ACTIVATION_PLAN.md`. Evidence: `docs/product/VIONA_MODERN_HOME_SHELL_MODE_RESOLVER_FOUNDATION_EVIDENCE.md`. |
