...
| Pack40 multi-tenant VionaRequest access enforcement (planning) | **REFINED v3 / PLANNING ONLY — Pack40A BLOCKED** — PR #340 merged @ `95031be`; corrective refinement branch supersedes unsafe v2 registry-absence semantics. **Three-state classification:** consumer-classified (positive proof only), merchant-classified (exact profile tenant equality), **unresolved-tenant** (fail closed — never consumer-by-default). **Rejected:** inferring consumer from absence in `MerchantProfile.tenantId` set. **Canonical consumer marker: UNKNOWN from source** — Pack19 create accepts client free-text; schema `tenantId String` non-null; no sentinel. **Pack40A implementation NOT authorized** until `APPROVE_PACK40A_READ_ONLY_TENANT_PROVENANCE_INVENTORY` (or equivalent) establishes consumer representation. Slices Pack40A–D + Pack40S retained; dual-role `VionaRequestPrincipalContext` retained; bounded query = `tenantId = M.tenantId OR tenantId = C` (no global NOT IN). See `docs/product/VIONA_PACK40_TENANT_SCOPE_ENFORCEMENT_PLAN.md`. |
