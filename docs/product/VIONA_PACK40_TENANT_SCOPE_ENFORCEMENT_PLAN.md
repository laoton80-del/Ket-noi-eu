...
| **Pack40D** | Indirect paths after per-path review | **Initial controlled merchant execution CLOSED/GREEN** (PR #375+#376; staging v27; sole trigger `internalAuthenticatedController`); signed-webhook DISABLED; `approvedInternalDispatch` UNWIRED; consumer/legacy UNSUPPORTED; **Pack40DR audit #377**; **Pack40DR1 schema foundation** (`leaseGeneration` + exact `providerExternalReference`; migration unapplied); recovery runtime still unimplemented; next after schema merge = separate staging migrate-apply; other Pack40D triggers/providers **not** closed |
