/**
 * Travel bottom nav actions match Local — QA captures (scroll to footer).
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-bottom-nav-actions-match-local'
);
...
const VIEWPORTS = [
  { name: 'travel-bottom-nav-390x844', width: 390, height: 844 },
  { name: 'travel-bottom-nav-844x390', width: 844, height: 390 },
  { name: 'travel-bottom-nav-768x1024', width: 768, height: 1024 },
  { name: 'travel-bottom-nav-1024x768', width: 1024, height: 768 },
  { name: 'travel-bottom-nav-1366x768', width: 1366, height: 768 },
  { name: 'travel-bottom-nav-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function openTravelRoute(page) {
  for (const route of ['/travel', '/tabs/travel', '/TravelHub']) {
    ...
    const ok = await page
      .waitForSelector('[data-testid="travel-bottom-escape-bar"]', { timeout: 45_000 })
      .then(() => true)
      .catch(() => false);
    if (ok) return route;
  }
...
async function scrollToBottomNav(page) {
  await page.evaluate(() => {
    const el = document.querySelector('[data-testid="travel-bottom-escape-bar"]');
