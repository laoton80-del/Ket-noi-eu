...
| FC-P0 Local provider authority E8 Case B B0 target and hosting decision | **READY FOR PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_B0_TARGET_AND_HOSTING_DECISION`; baseline `8ca1dbb…`; docs-only; decision `B0_DEDICATED_VERCEL_STAGING_PROJECT_RECOMMENDED_FOR_SEPARATE_B1_AUTHORIZATION`; no project created/bound; blockers preserved; B1–B7 unauthorized; E8–E10 unauthorized. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_B0_TARGET_AND_HOSTING_DECISION.md`. |
