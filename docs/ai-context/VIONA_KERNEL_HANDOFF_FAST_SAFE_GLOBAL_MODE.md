...
| FC-P0 Local request create client implementation | **READY FOR PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_REQUEST_CREATE_CLIENT`; baseline `1c631dc`; wires B2C client create to existing `POST /api/local/requests` (201); host `LocalUserRequestStatusScreen` + `LocalUserRequestCreateComposer`; `createUserLocalServiceRequest` via session JWT; fixed `source=LOCAL_SCREEN`; no backend/schema/payment/AI/deploy; `REQUEST_ONLY_NO_CHARGE` preserved; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred. Evidence: `docs/product/VIONA_FC_P0_LOCAL_REQUEST_CREATE_CLIENT_IMPLEMENTATION_EVIDENCE.md`. |
