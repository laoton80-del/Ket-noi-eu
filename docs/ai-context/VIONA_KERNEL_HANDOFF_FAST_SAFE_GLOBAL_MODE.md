...
| FC-P0 Local provider eligibility authority boundary + implementation plan | **READY FOR PLAN PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_BOUNDARY_AND_IMPLEMENTATION_PLAN`; baseline `19dbc2f`; `MINIMAL_SCHEMA_EXTENSION_REQUIRED`; owner `LOCAL_PROVIDER_AUTHORITY_OWNER_INTERNAL_OPS`; model `LocalProviderEligibility`; route `GET /api/local/providers`; create-service enforcement; two ordered packs (schema/backend then client wiring); **docs-only**; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; FC-P0 still blocked. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_BOUNDARY_AND_IMPLEMENTATION_PLAN.md`. |
