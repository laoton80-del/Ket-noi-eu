...
| FC-P0 Local provider authority timestamp / referential / PATCH finalization | **READY FOR FINALIZATION PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_TIMESTAMP_REFERENTIAL_PATCH_FINALIZATION`; baseline `d3bd2935b7ff8029eb5e4c96869c70f1bf1a54ac`; locks latest `activatedAt`, clear `suspendedAt` on reactivation, immutable `retiredAt`, Restrict retention graph (eligibility→Business, audit→eligibility, audit→User), REGISTERED prior null/`[]` discriminator, PATCH-by-status incl. RETIRED→409, executable A1 card; Pack A1 phrase **UNAUTHORIZED**; docs-only; FC-P0 still blocked; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_TIMESTAMP_REFERENTIAL_PATCH_FINALIZATION.md`. |
