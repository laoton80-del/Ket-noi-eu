...
| Mobile SOS shell consolidation Phase 1 | **IMPLEMENTATION / PR OPEN** — operator phrase `APPROVE_VIONA_MOBILE_SOS_SHELL_CONSOLIDATION_PHASE_1`; baseline audit `MOBILE_UI_LEGACY_IS_CURRENT_SOURCE`; floating absolute SOS FAB removed; SOS integrated into bottom-tab shell via `VionaGlobalSosShellAction` (3s hold preserved); canonical modal `src/screens/b2c/SOSModal.tsx` retained (single mount); Local/Travel/Academy rail SOS unchanged; fashion desktop command-bar SOS unchanged; ProfileSwitcher/SmartTrioLanguageChip floating chrome **deferred**; Wave 2 native-device confidence remains **NOT RUN**; Pack40DR wait-state preserved; Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_MOBILE_SOS_SHELL_CONSOLIDATION_PHASE_1_EVIDENCE.md`. |
