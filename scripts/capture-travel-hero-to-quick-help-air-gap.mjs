/**
 * Travel hero → quick-help air gap — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-hero-to-quick-help-air-gap'
);
...
const VIEWPORTS = [
  { name: 'travel-air-gap-1366x768', width: 1366, height: 768 },
  { name: 'travel-air-gap-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
  { name: 'travel-air-gap-1024x768', width: 1024, height: 768 },
  { name: 'travel-air-gap-390x844', width: 390, height: 844 },
];
...
async function captureViewport(browser, vp) {
  ...
  const routes = ['/travel', '/tabs/travel', '/TravelHub'];
  const route = await openRoute(page, routes, '[data-testid="travel-flagship-cards-row"]');
