...
| Pack40DR2 dormant recovery services | **IMPLEMENTED (dormant, not wired)** — PR #380 @ `e632d6a` (`2026-07-16T07:36:15Z`); operator phrase `APPROVE_PACK40DR2_DORMANT_RECOVERY_SERVICES`; dormant system-recovery principal; generation-fenced recovery lease CAS; exact-provider reconciliation; escrow reconciliation; recovered finalization; narrow repository CAS helpers; fake suite **85/85 PASS**; markers `PACK40DR2_DORMANT_RECOVERY_SERVICES_IMPLEMENTED` + `PACK40DR2_RECOVERY_RUNTIME_NOT_WIRED`; evidence `docs/product/VIONA_PACK40DR2_DORMANT_RECOVERY_SERVICES_EVIDENCE.md`. **Not** CLOSED/GREEN — recovery runtime unwired. |
| Pack40DR3A live generation fencing + provider reference hardening | **IMPLEMENTED (live path hardened, recovery still unwired)** — branch `feat/pack40dr3a-live-fencing-provider-reference`, operator phrase `APPROVE_PACK40DR3A_LIVE_FENCING_PROVIDER_REFERENCE_HARDENING`, master @ `e632d6a` (PR #380); live Pack40D2/D3A/D3B propagate/require `leaseGeneration` on all post-claim mutations; exact `providerExternalReference` persisted from bounded Twilio adapter on durable outcome record; stale-generation CAS rejects worker races; no recovery endpoint/scheduler/worker; no Pack40DR2 runtime import; no DB/staging/live provider/escrow/deploy/schema change; markers `PACK40DR3A_LIVE_GENERATION_FENCING_WIRED` + `PACK40DR3A_PROVIDER_REFERENCE_RUNTIME_POPULATION_WIRED`; initial controlled Pack40D CLOSED/GREEN preserved; signed-webhook DISABLED; `approvedInternalDispatch` UNWIRED; consumer/legacy UNSUPPORTED; Pack40S UNIMPLEMENTED. Evidence: `docs/product/VIONA_PACK40DR3A_LIVE_FENCING_PROVIDER_REFERENCE_EVIDENCE.md`. Next: separately authorize Pack40DR3B operator recovery endpoint. |
