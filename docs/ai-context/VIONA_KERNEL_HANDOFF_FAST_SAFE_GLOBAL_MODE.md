...
| Pack40 multi-tenant VionaRequest access enforcement (planning) | **OPEN / PLANNING ONLY** — branch `docs/pack40-tenant-scope-enforcement-planning` — docs-only design closing the Pack34-deferred gap: `buildAuthorizedVionaRequestWhere()`'s optional `expectedTenantId` exists but is **not wired** at any of the 4 production call sites (read list, read detail, note, status-action); verified read-only against merged master @ `587dcd195e5bf5fd66415576e2a6cbd621cc2eed`. Proposes tagged server-side `VionaRequestAccessContext` (consumer vs merchant), resolver from `findMerchantProfileByOwnerUserId()`, layered controller+service enforcement, unified non-leaking `request_not_found` denial, **no schema change**, webhook/dispatch/orchestrator/escrow untouched. **No implementation authorized** — phrase `APPROVE_PACK40_TENANT_SCOPE_ENFORCEMENT_IMPLEMENTATION` reserved for a future increment. See `docs/product/VIONA_PACK40_TENANT_SCOPE_ENFORCEMENT_PLAN.md`. |
