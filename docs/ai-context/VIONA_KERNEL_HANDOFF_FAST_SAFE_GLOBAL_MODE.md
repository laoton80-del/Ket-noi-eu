...
| Pack40P2D staging create-path deploy (execution-only) | **CLOSED / GREEN** — branch `docs/pack40p2d-staging-create-path-provenance-deploy`, operator phrase `APPROVE_PACK40P2D_STAGING_CREATE_PATH_PROVENANCE_DEPLOY` — deployed merged master @ `37d5bb3` to `viona-api-staging-eu` (**v22→v23**, image `deployment-01KXH70SC5G1KKJYWG5KYB5SDX`); pre/post health **HTTP 200**; **no migration, DB, request QA, backfill, secret change, production**. State: **`STAGING_SCHEMA_AND_CREATE_PATH_CODE_DEPLOYED`** / **`PROVENANCE_DATA_VERIFICATION_STILL_REQUIRED`**. Evidence: `docs/product/VIONA_PACK40P2D_STAGING_CREATE_PATH_PROVENANCE_EVIDENCE.md`. Pack40A **blocked**. Next: **Pack40P5 provenance verification** (separately authorized). |
