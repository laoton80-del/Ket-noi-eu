...
| Pack40DR1 recovery schema foundation | **IMPLEMENTED locally (migration unapplied)** — operator phrase `APPROVE_PACK40DR1_RECOVERY_SCHEMA_PACKET`; additive `leaseGeneration Int @default(0)` + nullable exact `providerExternalReference VarChar(191)` on `VionaRequestExecutionAttempt`; migration-managed partial unique `(providerName, providerExternalReference) WHERE providerExternalReference IS NOT NULL`; digests retained; **no** recovery principal/service/endpoint/scheduler; **no** D2/D3/Twilio/escrow runtime writes; **no** DB/staging/provider/escrow/deploy; Pack40A/B/C CLOSED/GREEN; initial Pack40D controlled merchant execution CLOSED/GREEN; signed-webhook DISABLED; internal dispatch UNWIRED; consumer/legacy UNSUPPORTED; Pack40S UNIMPLEMENTED. Evidence: `docs/product/VIONA_PACK40DR1_RECOVERY_SCHEMA_FOUNDATION_EVIDENCE.md`. Next after merge: separate staging migration-apply authorization. |
