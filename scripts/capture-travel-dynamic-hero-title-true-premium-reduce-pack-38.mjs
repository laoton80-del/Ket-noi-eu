/**
 * Pack 38 — Dynamic Hero title true premium reduce QA.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-dynamic-hero-title-true-premium-reduce-pack-38'
);
...
const TARGETS = {
  normal: { fontSizeMin: 52, fontSizeMax: 58, lineHeightMin: 56, lineHeightMax: 62, maxWidthMin: 620, maxWidthMax: 700 },
  fullscreen: { fontSizeMin: 56, fontSizeMax: 62, lineHeightMin: 60, lineHeightMax: 66, maxWidthMin: 680, maxWidthMax: 760 },
};
...
async function openTravel(page) {
  await page.goto(`${BASE}/travel?pack38=${BUST}`, {
...
async function collectTitleMetrics(page, mode) {
  return page.evaluate(({ modeLabel, targets }) => {
    ...
    return {
      ...
      failGuardNormal: modeLabel.includes('normal') ? fontSizePx != null && fontSizePx >= 64 : false,
      failGuardFullscreen: modeLabel.includes('fullscreen') ? fontSizePx != null && fontSizePx >= 70 : false,
    };
...
const proof = {
  pack: 38,
  beforeReference: {
    normal: { fontSizePx: 66, lineHeightPx: 68, maxWidthPx: 720 },
    fullscreen: { fontSizePx: 68, lineHeightPx: 70, maxWidthPx: 760 },
  },
  targets: TARGETS,
  captures: {},
};
...
console.log(`Pack 38 evidence → ${OUT_DIR}`);
