...
| FC-P0 Local provider eligibility authority boundary + implementation plan | **MERGED ON MASTER** — PR #415 @ `dc5c625`; architecture locked; post-merge strict review **blocked** on lifecycle/write/audit/consistency — see remediation row. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_BOUNDARY_AND_IMPLEMENTATION_PLAN.md`. |
| FC-P0 Local provider authority plan lifecycle/write/audit/consistency remediation | **READY FOR PLAN REMEDIATION PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_LIFECYCLE_WRITE_AUDIT_CONSISTENCY_REMEDIATION`; baseline `dc5c625`; locks transition table, RETIRED terminal, Role.ADMIN via superAdminMiddleware, exact ops routes, eligibility audit model, create-in-transaction, A1→A2→B; immediate phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_SCHEMA_DOMAIN_ENFORCEMENT` **UNAUTHORIZED**; docs-only; FC-P0 still blocked; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_LIFECYCLE_WRITE_AUDIT_CONSISTENCY_REMEDIATION.md`. |
