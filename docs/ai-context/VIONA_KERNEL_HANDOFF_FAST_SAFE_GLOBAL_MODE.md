...
| FC-P0 Local create failure-code envelope + Pack B recovery discrimination | **READY FOR PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_CREATE_FAILURE_CODE_ENVELOPE_AND_PACK_B_RECOVERY_DISCRIMINATION`; baseline `5ef5a1d58539648311548db2c9f6017440f088f4`; branch `fix/viona-fc-p0-local-create-failure-code-envelope-pack-b-recovery`; Local create fail envelope adds allowlisted `code`; recovery only for `provider_not_available` (404) + `service_type_not_supported` (400); C1–C20 + remediated B11/B12 PASS; **no** migrate-apply / deploy / activation / live QA; FC-P0 still blocked pending execution gates; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_CREATE_FAILURE_CODE_ENVELOPE_AND_PACK_B_RECOVERY_DISCRIMINATION.md`. |
| FC-P0 Local provider Pack B — client authority wiring | **MERGED on master @ 5ef5a1d** (PR #422) — superseded for create-recovery by failure-code envelope remediation above; prior evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_CLIENT_WIRING_IMPLEMENTATION_EVIDENCE.md`. |
