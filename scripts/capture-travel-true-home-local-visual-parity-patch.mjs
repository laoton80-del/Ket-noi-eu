/**
 * Travel true Home/Local visual parity patch QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-true-home-local-visual-parity-patch'
);
...
const VIEWPORTS = [
  { name: 'travel-visual-parity-390x844', width: 390, height: 844 },
  { name: 'travel-visual-parity-844x390', width: 844, height: 390 },
  { name: 'travel-visual-parity-768x1024', width: 768, height: 1024 },
  { name: 'travel-visual-parity-1024x768', width: 1024, height: 768 },
  { name: 'travel-visual-parity-1366x768', width: 1366, height: 768 },
  { name: 'travel-visual-parity-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function waitForTravelOpeningStack(page) {
  await page.waitForSelector('[data-testid="travel-quick-help-translation"]', { timeout: 60_000 });
  await page.waitForSelector('[data-testid="travel-pilot-strip"]', { timeout: 15_000 });
  await page.waitForSelector('[data-testid="travel-connected-local"]', { timeout: 15_000 });
}
...
async function main() {
  ...
  try {
    for (const vp of VIEWPORTS) {
      ...
      await waitForTravelOpeningStack(page);
