/**
 * Travel secondary cinematic scene card — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-secondary-cinematic-scene-card'
);
...
const VIEWPORTS = [
  { name: 'travel-scene-390x844', width: 390, height: 844 },
  { name: 'travel-scene-768x1024', width: 768, height: 1024 },
  { name: 'travel-scene-1024x768', width: 1024, height: 768 },
  { name: 'travel-scene-1366x768', width: 1366, height: 768 },
  { name: 'travel-scene-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function scrollForViewport(page, mobile) {
  await page.evaluate(({ mobile: isMobile }) => {
    ...
    const target = isMobile ? business ?? connected : document.querySelector('[data-testid="travel-local-assist-card"]');
    target?.scrollIntoView({ block: isMobile ? 'end' : 'center', behavior: 'instant' });
    if (isMobile) window.scrollBy(0, 160);
  }, { mobile });
...
async function measureMobileOverlap(page) {
  return page.evaluate(() => {
    const chip = document.querySelector('[data-testid="travel-connected-business"]');
    const local = document.querySelector('[data-testid="travel-connected-local"]');
    const chipRect = chip?.getBoundingClientRect();
    const localRect = local?.getBoundingClientRect();
    const vh = window.innerHeight;
    const floatReserve = 200;
    return {
      viewport: vh,
      businessBottom: chipRect ? Math.round(chipRect.bottom) : null,
      localBottom: localRect ? Math.round(localRect.bottom) : null,
      businessClearance: chipRect ? Math.round(vh - chipRect.bottom) : null,
      localClearance: localRect ? Math.round(vh - localRect.bottom) : null,
      passesFloatReserve: chipRect ? vh - chipRect.bottom >= floatReserve : null,
    };
  });
}
