/**
 * Home dynamic Travel hero daylight asset swap QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-home-dynamic-travel-hero-daylight-asset-swap'
);
...
const VIEWPORTS = [
  { name: 'home-travel-hero-daylight-390x844', width: 390, height: 844 },
  { name: 'home-travel-hero-daylight-844x390', width: 844, height: 390 },
  { name: 'home-travel-hero-daylight-768x1024', width: 768, height: 1024 },
  { name: 'home-travel-hero-daylight-1024x768', width: 1024, height: 768 },
  { name: 'home-travel-hero-daylight-1366x768', width: 1366, height: 768 },
];
...
async function hoverTravelWorldCard(page) {
  const hovered = await page.evaluate(() => {
    const labels = ['Cestování', 'Travel', 'TRAVEL'];
    const nodes = [...document.querySelectorAll('div, span, p')].filter(
      (el) => el instanceof HTMLElement && labels.some((l) => el.textContent?.trim() === l)
    );
    const card =
      nodes
  ...
  if (!hovered) {
    const coords = await page.evaluate(() => {
      ...
      return { x: Math.round(24 + slot * 1.5), y: Math.round(window.innerHeight - 180) };
    });
    await page.mouse.move(coords.x, coords.y);
  }
  await page.waitForFunction(
    () =>
      document.body.textContent?.includes('VIONA TRAVEL') ||
      document.body.textContent?.includes('Move across countries'),
    { timeout: 15_000 }
  ).catch(() => {});
  await page.waitForTimeout(900);
}

async function waitForTravelHeroImage(page) {
  await page.waitForFunction(
    () => {
      const imgs = [...document.querySelectorAll('img')];
      return imgs.some((img) => {
        const src = img.getAttribute('src') ?? '';
        return src.includes('viona-home-travel-hero-daylight');
      });
    },
    { timeout: 60_000 }
  ).catch(() => {});
...
async function main() {
  ...
  try {
    for (const vp of VIEWPORTS) {
      ...
      await hoverTravelWorldCard(page);
      await waitForTravelHeroImage(page);
      await page.waitForTimeout(600);
