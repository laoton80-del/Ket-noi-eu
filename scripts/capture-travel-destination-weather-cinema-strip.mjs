/**
 * Travel destination weather cinema strip — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-destination-weather-cinema-strip'
);
...
const VIEWPORTS = [
  { name: 'weather-cinema-390x844', width: 390, height: 844 },
  { name: 'weather-cinema-768x1024', width: 768, height: 1024 },
  { name: 'weather-cinema-1024x768', width: 1024, height: 768 },
  { name: 'weather-cinema-1366x768', width: 1366, height: 768 },
  { name: 'weather-cinema-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function openTravelRoute(page) {
  for (const route of ['/travel', '/tabs/travel', '/TravelHub']) {
    ...
    const ok = await page
      .waitForFunction(
        () => Boolean(document.querySelector('[data-testid="travel-destination-weather-cinema-strip"]')),
        { timeout: 60_000 }
      )
      .then(() => true)
      .catch(() => false);
  ...
  throw new Error('Travel destination weather cinema strip readiness failed.');
}
...
async function measureDestinationStrip(page) {
  return page.evaluate(() => {
    const strip = document.querySelector('[data-testid="travel-destination-weather-cinema-strip"]');
    const fx = document.querySelector('[data-testid="travel-destination-fx-strip"]');
    ...
    return {
      ...
      demoNote: strip?.textContent?.includes('dữ liệu mẫu') ?? false,
...
async function main() {
  ...
  try {
    for (const vp of VIEWPORTS) {
      ...
      const stripMetrics = await measureDestinationStrip(page);
      console.log(JSON.stringify({ viewport: vp.name, stripMetrics }, null, 2));
      if (vp.width === 390) {
