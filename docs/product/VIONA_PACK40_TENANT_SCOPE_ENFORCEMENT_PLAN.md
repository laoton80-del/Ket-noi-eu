...
| **Pack40D** | Indirect paths after per-path review | **Initial controlled merchant execution CLOSED/GREEN** (PR #375+#376; staging v27; sole trigger `internalAuthenticatedController`); signed-webhook DISABLED; `approvedInternalDispatch` UNWIRED; consumer/legacy UNSUPPORTED; **Pack40DR audit #377**; **Pack40DR1 schema APPLIED to staging** (PR #378+#379); **Pack40DR2 dormant recovery services implemented locally** (principal, lease CAS, provider/escrow reconciliation, recovered finalization; **not wired** — `PACK40DR2_RECOVERY_RUNTIME_NOT_WIRED`; `providerExternalReference` runtime population **not wired**); next = separately authorize Pack40DR3; other Pack40D triggers/providers **not** closed |
