...
| Mobile SOS shell consolidation Phase 1 | **MERGED / FINAL GATE `BLOCKED_GLOBAL_SOS_REACHABILITY`** — PR #392 squash-merged to master @ `95c41d2` (`2026-07-17T08:34:57Z`); feature HEAD `acbc607`; floating FAB removed; B2C primary surfaces retain one shell/rail/command SOS; canonical `screens/b2c/SOSModal` single mount; EmergencySOS preserved; desktop left-rail B2B/Broker/Admin lose SOS entry (no tab-bar host when `tabBarPosition===left`); ProfileSwitcher/language floating **deferred**; Wave 2 native-device confidence remains **NOT RUN**; Pack40DR wait-state preserved; Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_MOBILE_SOS_SHELL_CONSOLIDATION_PHASE_1_EVIDENCE.md`. |
