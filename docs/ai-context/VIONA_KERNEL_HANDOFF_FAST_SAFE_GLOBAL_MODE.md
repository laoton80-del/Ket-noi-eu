...
### DEFERRED / BACKLOG (recorded, not authorized — logged during Pack37 implementation sync, updated during Pack38 implementation sync with 2 additional findings)

Explicitly logged here per operator instruction so no item is lost between sessions. None of the 4
items below is authorized by this log entry — each requires its own future, separately-authorized
pack.
...
3. **Test-isolation gap: a real, billed OpenAI call fires during local regression** — found during
   Pack38's full regression run. `scripts/test-viona-pack37-b2b-dispatcher-realization.ts`'s own
   pre-existing test #19 ("secrets: end-to-end dispatch with `OPENAI_API_KEY` unset...", unchanged by
   Pack38) `delete`s `process.env.OPENAI_API_KEY` before calling dispatch, but that same test is the
   only one in the file that reaches the real, unmocked `executeMerchantReadOnlyQuery` →
   `findMerchantProfileById` → the process's first-ever `getPrisma()` call — and `@prisma/client`'s
   own `PrismaClient` construction auto-loads `.env` internally (independent of any application-level
   `dotenv` import), silently re-injecting the real key mid-test and defeating the test's own
   env-isolation. This went unnoticed before Pack38 only because `.env`'s `OPENAI_API_KEY` was an
   empty string until the `APPROVE_PACK37_OPENAI_STAGING_SECRET` session. The test's own assertions
   still pass either way (Tier-1 or Tier-2 both satisfy `dispatchAccepted:true` + non-empty
   `replyText`), so this is a test-isolation gap, not a functional regression. Fix would be a small,
   mechanical fake-`executeMerchantQuery` injection into that one test, mirroring its own sibling
   tests 1–4 in the same file. Not fixed by Pack38 — deliberately left to a future, dedicated,
   standalone task rather than expanding this pack's already-flagged file-allowlist deviation
   further.
4. **Double LLM classification call per webhook message** — found during Pack38's live-staging `fly
   logs` capture. The merchant-agent webhook controller's own `routeIntent()` pre-check and
   `dispatchVionaAutonomousRequest()`'s internal re-classification are 2 independent,
   identical-input LLM calls for the same single message — a pre-existing Pack35/37 design, not
   introduced by Pack38 (only became visible in the logs because Pack38's larger, few-shot-bearing
   prompt made the 2 calls' token cost obvious). A minor cost-efficiency backlog item, not a
   correctness bug; a candidate future fix would thread the already-computed classification decision
   from `routeIntent()` into `dispatch()` instead of re-deriving it.
