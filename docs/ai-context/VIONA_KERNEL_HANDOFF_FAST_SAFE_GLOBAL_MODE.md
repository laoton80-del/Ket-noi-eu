...
| Pack40B tenant note mutation enforcement (implementation) | **IMPLEMENTATION PR REVIEW** — operator phrase `APPROVE_PACK40B_TENANT_NOTE_ENFORCEMENT` — provenance-aware note mutation on `POST /api/viona/requests/:id/actions/note` only; reuses server-owned `resolveVionaRequestReadPrincipalContext`; `buildAuthorizedVionaRequestNoteWhere` with consumer branch + active-merchant branch (`isActive=true`); `legacyUnresolved` fail-closed; transactional authorize-then-write; **64/64** Pack40B tests + Pack40A **39/39** regression PASS; **no schema/migration/DB/staging/deploy**; Pack40A read unchanged; Pack40C/D/S not implemented. Evidence: `docs/product/VIONA_PACK40B_TENANT_NOTE_ENFORCEMENT_EVIDENCE.md`. Next: separately authorize `APPROVE_PACK40BD_STAGING_NOTE_ENFORCEMENT_DEPLOY` after merge. |
