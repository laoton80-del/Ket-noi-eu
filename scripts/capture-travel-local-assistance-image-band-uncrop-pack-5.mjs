/**
 * Travel Local Assistance — IMAGE_BAND_UNCROP PACK_5 QA.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-local-assistance-image-band-uncrop-pack-5'
);
...
const VIEWPORTS = [
  { name: 'local-assist-pack5-390x844', width: 390, height: 844 },
  { name: 'local-assist-pack5-768x1024', width: 768, height: 1024 },
  { name: 'local-assist-pack5-1024x768', width: 1024, height: 768 },
  { name: 'local-assist-pack5-1366x768', width: 1366, height: 768 },
  { name: 'local-assist-pack5-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function measure(page) {
  return page.evaluate(() => {
    ...
    const dock = document.querySelector('[data-testid="travel-local-discovery-category-row"]')?.parentElement;
    ...
    return {
      ...
      sceneOverflow: scene ? window.getComputedStyle(scene).overflow : null,
      bgBackgroundPosition: bgStyle?.backgroundPosition ?? null,
      bgBackgroundSize: bgStyle?.backgroundSize ?? null,
      ...
      hasForbiddenClaim: /\bđặt ngay\b|\bgọi fixer\b|\blive GPS\b/i.test(text),
    };
  });
}
