...
| Pack40P4D merchant backfill dry run (read-only) | **READY FOR PR REVIEW** — branch `chore/pack40p4-merchant-backfill-dry-run`, operator phrase `APPROVE_PACK40P4_MERCHANT_BACKFILL_DRY_RUN` — staging read-only candidate discovery: **5/5** fully qualified merchant candidates, digest `aa74f638…930213`, **0** ambiguity/owner-mismatch/duplicate-webhook; **P4W design-ready**; **no write, no consumer backfill, no deploy, no Pack40A**. Evidence: `docs/product/VIONA_PACK40P4_MERCHANT_BACKFILL_DRY_RUN_EVIDENCE.md`. P4W **not authorized**. Next: **`APPROVE_PACK40P4_STAGING_MERCHANT_BACKFILL_WRITE`** or **Pack40P5** (separately authorized). |
