...
| FC-P0 Local provider Pack A2 — deterministic behavioral evidence remediation | **READY FOR REMEDIATION PR REVIEW** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_A2_DETERMINISTIC_BEHAVIORAL_EVIDENCE_REMEDIATION`; baseline `ff0fdfa7752eacb265e94f1de75e676d57ae5c62`; branch `fix/viona-fc-p0-local-provider-a2-deterministic-behavioral-evidence-remediation`; production Prisma-default DI seam + scripts-only deterministic store/txn/clock; cases 30–43 + rollback A–D executed without migration apply; optional DB skip separate; **no** Pack B / migrate-apply / deploy / activation; FC-P0 still blocked; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_A2_DETERMINISTIC_BEHAVIORAL_EVIDENCE_REMEDIATION.md`. |
