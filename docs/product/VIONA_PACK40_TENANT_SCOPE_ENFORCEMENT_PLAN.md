...
| **Pack40D** | Indirect paths after per-path review | **Initial controlled merchant execution CLOSED/GREEN** (PR #375+#376; staging v27; sole trigger `internalAuthenticatedController`); signed-webhook DISABLED; `approvedInternalDispatch` UNWIRED; consumer/legacy UNSUPPORTED; **Pack40DR audit #377**; **Pack40DR1 schema APPLIED to staging** (PR #378+#379); **Pack40DR2 dormant recovery services merged**; **Pack40DR3A live generation fencing + exact `providerExternalReference` population wired** (`PACK40DR3A_LIVE_GENERATION_FENCING_WIRED`); **Pack40DR3B operator internal recovery endpoint implemented locally** (`PACK40DR3B_OPERATOR_INTERNAL_RECOVERY_ENDPOINT_WIRED`, `PACK40DR2_RECOVERY_RUNTIME_WIRED` — one exact-attempt route only; no scheduler/worker); recovery **not** CLOSED/GREEN; next = separately authorize Pack40DRD staging deployment; other Pack40D triggers/providers **not** closed |
