...
| Mobile SOS shell consolidation Phase 1 | **MERGED WITH LEFT-RAIL GAP** — PR #392 squash `95c41d2` (`2026-07-17T08:34:57Z`); floating FAB removed; B2C primary surfaces retained one shell/rail/command SOS; desktop left-rail B2B/Broker/Admin initially had **0** SOS (`tabBarPosition===left` skip); final gate classification `BLOCKED_GLOBAL_SOS_REACHABILITY`; Wave 2 native confidence remains **NOT RUN**. Evidence: `docs/product/VIONA_MOBILE_SOS_SHELL_CONSOLIDATION_PHASE_1_EVIDENCE.md`. |
| Mobile SOS left-rail reachability remediation | **IMPLEMENTATION / PR PENDING REVIEW** — operator phrase `APPROVE_VIONA_MOBILE_SOS_LEFT_RAIL_REACHABILITY_REMEDIATION`; restores integrated `VionaGlobalSosShellAction` in left-rail chrome for B2B/Broker/Admin; no floating overlay; consumer Phase-1 behavior preserved; canonical `screens/b2c/SOSModal` single mount; profile/language Phase 2 **not started**; Pack40DR wait-state preserved; Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_MOBILE_SOS_LEFT_RAIL_REACHABILITY_REMEDIATION_EVIDENCE.md`. |
