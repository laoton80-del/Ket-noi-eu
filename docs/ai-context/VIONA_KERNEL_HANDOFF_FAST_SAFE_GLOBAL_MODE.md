...
| Pack40 multi-tenant VionaRequest access enforcement (planning) | **REFINED v3 / Pack40A BLOCKED behind Pack40P** — PR #340–#342 merged @ `a4619a1`. Inventory: 10 staging requests, 5 unresolved, **`canonicalConsumerProvenanceConfirmed: false`**. **Pack40P planning** (`docs/product/VIONA_PACK40P_REQUEST_PROVENANCE_MODEL_PLAN.md`): additive `VionaRequestScopeKind` (`consumer`/`merchant`/`legacyUnresolved`), `scopeKind` DB default fail-safe `legacyUnresolved`, optional `merchantProfileId` FK, Pack40P1–P5 sliced execution. **No schema/product code authorized by planning.** Pack40A remains blocked until Pack40P1–P5 definition-of-ready. Access plan: `docs/product/VIONA_PACK40_TENANT_SCOPE_ENFORCEMENT_PLAN.md`. |
