/**
 * Travel final parity polish — hero text, secondary panels, connected links QA.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-final-parity-polish-hero-secondary-and-connected'
);
...
const VIEWPORTS = [
  { name: 'travel-final-parity-390x844', width: 390, height: 844 },
  { name: 'travel-final-parity-844x390', width: 844, height: 390 },
  { name: 'travel-final-parity-768x1024', width: 768, height: 1024 },
  { name: 'travel-final-parity-1024x768', width: 1024, height: 768 },
  { name: 'travel-final-parity-1366x768', width: 1366, height: 768 },
  { name: 'travel-final-parity-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function waitForTravelHub(page) {
  await page.waitForSelector('[data-testid="travel-flagship-translation"]', { timeout: 60_000 });
  await page.waitForSelector('[data-testid="travel-hero-lighting-network"]', { timeout: 15_000 });
  await page.waitForSelector('[data-testid="travel-perspective-cards-row"]', { timeout: 15_000 });
}
...
async function scrollToSecondaryZone(page) {
...
async function main() {
  ...
  try {
    for (const vp of VIEWPORTS) {
      ...
      await waitForTravelHub(page);
      if (vp.width >= 768) {
        await scrollToSecondaryZone(page);
      }
