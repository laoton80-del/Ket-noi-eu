...
export type VionaRequestStatusMerchantProfileResolution = 'none' | 'single' | 'ambiguous';

export type VionaRequestStatusPrincipalContext = Readonly<{
  authUserId: string;
  ...
  merchantProfileResolution: VionaRequestStatusMerchantProfileResolution;
}>;
...
export type StatusPrincipalMerchantProfileClient = Pick<
  Prisma.TransactionClient,
  'merchantProfile'
>;

export type ResolveVionaRequestStatusPrincipalContextDeps = Readonly<{
...
/**
 * Resolve server-owned status-mutation principal context inside an interactive transaction.
 * Performs at most one bounded MerchantProfile lookup by unique ownerUserId on the tx client.
 */
export async function resolveVionaRequestStatusPrincipalContext(
  authUserId: string,
  tx: StatusPrincipalMerchantProfileClient,
  deps: ResolveVionaRequestStatusPrincipalContextDeps = {},
): Promise<VionaRequestStatusPrincipalContext> {
