...
| Pack40DR3B operator internal recovery endpoint | **IMPLEMENTED (local wiring only, not deployed)** — branch `feat/pack40dr3b-operator-internal-recovery-endpoint`, operator phrase `APPROVE_PACK40DR3B_OPERATOR_INTERNAL_RECOVERY_ENDPOINT`, master @ `219f47b` (PR #381); `POST /api/internal/viona/execution-attempts/:attemptId/recovery` with JWT + `Role.ADMIN`; DR2 coordinator wires lease → provider recon → escrow recon → recovered finalize; read-only Twilio status lookup adapter isolated from send; no scheduler/worker/scan; no DB/staging/live provider/escrow/deploy/schema change; markers `PACK40DR3B_OPERATOR_INTERNAL_RECOVERY_ENDPOINT_WIRED` + `PACK40DR2_RECOVERY_RUNTIME_WIRED`; initial controlled Pack40D CLOSED/GREEN preserved; recovery **not** CLOSED/GREEN; signed-webhook DISABLED; `approvedInternalDispatch` UNWIRED; consumer/legacy UNSUPPORTED; Pack40S UNIMPLEMENTED. Evidence: `docs/product/VIONA_PACK40DR3B_OPERATOR_INTERNAL_RECOVERY_ENDPOINT_EVIDENCE.md`. Next: separately authorize Pack40DRD staging deployment. |
