...
const PORT = Number(process.env.EXPO_CAPTURE_PORT || 8287);
...
async function main() {
  ...
  try {
    ...
    await page1366.screenshot({
      path: path.join(__dirname, 'screenshot-hero-airport-1366x768.png'),
      fullPage: false,
    });
    ...
    for (const vp of [
      ...
      await page.screenshot({
        path: path.join(__dirname, `screenshot-default-${vp.name}.png`),
        fullPage: false,
      });
      shots.push(vp.name);
      await page.close();
