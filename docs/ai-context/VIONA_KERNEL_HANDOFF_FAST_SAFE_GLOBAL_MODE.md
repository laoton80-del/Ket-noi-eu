...
| FC-P0 Local provider eligibility + Tourism coupling remediation | **READY FOR CONTAINMENT PR REVIEW — FC-P0 STILL BLOCKED** — operator phrase `APPROVE_VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_AND_TOURISM_COUPLING_REMEDIATION`; baseline `8bc568b`; Path 2: remove Tourism discover from Local create; honest `PROVIDER_SELECTION_UNAVAILABLE`; preserve UUID removal + submit guard; **no** `VALID_LOCAL_PROVIDER_AUTHORITY` in source; no backend/catalog; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_AND_TOURISM_COUPLING_REMEDIATION_EVIDENCE.md`. |
