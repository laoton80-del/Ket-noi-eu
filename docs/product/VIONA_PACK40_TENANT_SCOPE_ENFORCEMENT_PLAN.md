...
| **Pack40D** | Indirect paths after per-path review | **Initial controlled merchant execution CLOSED/GREEN** (PR #375+#376; staging **v28**; sole trigger `internalAuthenticatedController`); signed-webhook DISABLED; `approvedInternalDispatch` UNWIRED; consumer/legacy UNSUPPORTED; **Pack40DR audit #377**; **Pack40DR1 schema APPLIED to staging**; **Pack40DR2/DR3A/DR3B merged**; **operator recovery endpoint DEPLOYED to staging** (`PACK40DR_OPERATOR_RECOVERY_DEPLOYED_TO_STAGING`, release **v28**, image `deployment-01KXN3M9E6NWTFAE5YMW60T9FH`); endpoint **not invoked**; live recovery QA still required; recovery **not** CLOSED/GREEN; no scheduler/worker; other Pack40D triggers/providers **not** closed |
