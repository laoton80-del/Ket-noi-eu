/**
 * Travel secondary zone mobile/desktop premium fix — QA captures.
...
const OUT_DIR = path.join(
  ...
  'design',
  'evidence',
  'wave-3b-travel-secondary-premium-fix-mobile-desktop'
);
...
const VIEWPORTS = [
  { name: 'travel-secondary-fix-390x844', width: 390, height: 844 },
  { name: 'travel-secondary-fix-768x1024', width: 768, height: 1024 },
  { name: 'travel-secondary-fix-1024x768', width: 1024, height: 768 },
  { name: 'travel-secondary-fix-1366x768', width: 1366, height: 768 },
  { name: 'travel-secondary-fix-1366x768-fullscreen', width: 1366, height: 768, fullscreen: true },
];
...
async function scrollSecondaryZone(page, mobile) {
  await page.evaluate(({ mobile: isMobile }) => {
    const target =
      document.querySelector('[data-testid="travel-local-assist-card"]') ??
      document.querySelector('[data-testid="travel-secondary-zone"]');
    target?.scrollIntoView({ block: isMobile ? 'center' : 'center', behavior: 'instant' });
  }, { mobile });
...
async function main() {
  ...
  try {
    for (const vp of VIEWPORTS) {
      ...
      await scrollSecondaryZone(page, vp.width < 768);
