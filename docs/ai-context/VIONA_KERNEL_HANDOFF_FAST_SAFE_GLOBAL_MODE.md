...
| PR #470 branch update (one-off non-profile exception; historical provenance) | `COMPLETE`; resulting PR #470 head `df1c758cee318ba2f01bfdd39ce4c118da75c10e`, tree `c6fb164ef83886534c8d50e15f848828748f57d0`; incorporates master `3c542ccb2542357812054ddb40b7b85613d21a53`; canonical master not moved |
...
|-------------------------------------|----------------|
| Final published PR head | `34e012917900cddcab5fde8a9287887de12cea66`; sole parent `df1c758cee318ba2f01bfdd39ce4c118da75c10e` |
| Final tree | `f5e5653581b4401c9e5b4aa29a760aaec5dbbf70` |
| Final Kernel blob | `b0f3736ee874d2cd6205a82fb5e3f51f0e111d8f`; mode `100644`; raw length `685031` bytes |
| Kernel raw SHA-256 | `d9e088ac9adcad55f3e6a4397bc88e4bf7720c7371c4c4c97e9ee02699cac34f` |
| Finite-replace-v2 publication authority | `CONSUMED / NON-REUSABLE` |
| Durable closure | `COMPLETE / INDEPENDENTLY VERIFIED` |

| PR #470 review / thread closure | Verified state |
|---------------------------------|----------------|
| Review thread `PRRT_kwDOSFpBeM6lBWIw` | `RESOLVED`; resolved by `laoton80-del` |
| Final unresolved thread count | `0` |
| Final exact-head approver | `Maty2016`; `APPROVED` on `34e012917900cddcab5fde8a9287887de12cea66` |
| Earlier review/thread states | Historical pre-publication evidence only (see the PR #470 pre-publication snapshots below) |

| PR #470 two-stage gate / ledger evidence | Verified state |
|------------------------------------------|----------------|
| Stage 1 | `Viona Merge Readiness Gate`; check run `107649806357`; head `34e012917900cddcab5fde8a9287887de12cea66`; `SUCCESS`; `STAGE1_DISPATCH_AUTHORIZED = CONSUMED / NON-REUSABLE` |
| Stage 2 | `Viona Explicit Merge Authorization`; check run `107666589849`; head `34e012917900cddcab5fde8a9287887de12cea66`; `SUCCESS`; `STAGE2_DISPATCH_AUTHORIZED = CONSUMED / NON-REUSABLE` |
| Stage 2 authorization ID | `95a1e068-4ed0-46eb-a814-211ad707e036` |
| Stage 2 target key | `a5299098b5e14ca7a74fca61bb190059acb6fbeff99d1c1c5011ad7281903312` |
| Stage 2 ledger lifecycle | `ISSUED → CONSUMING → CONSUMED`; ledger commits `b6515b9c7f235c3c76c399486b3f342f3e591e7d` → `097be274a7bcc36fc1bec68e04bd5c834635ee6f` → `c243f19917b1e6893f3232790ebdba3b5405df4f` |
| Final ledger state | `CONSUMED / NON-REUSABLE`; record blob `394f7505d444e5b13335b2893ab0d3ffbf5fcd64`; ledger ref tip `c243f19917b1e6893f3232790ebdba3b5405df4f`; `merge_commit_sha = 9ba4a3f8f31799221b9025f03eedee11857b7cad` |

| PR #470 guarded merge | Verified state |
|-----------------------|----------------|
| Guarded merge wrapper | `scripts/viona-guarded-pr-merge.mjs`; one execution; one `squash` merge API call; no auto-merge, replay, retry, or push |
| Squash merge commit | `9ba4a3f8f31799221b9025f03eedee11857b7cad`; merged `2026-09-24T14:11:50Z` by `laoton80-del` |
| Sole squash parent | `3c542ccb2542357812054ddb40b7b85613d21a53` |
| Merged tree | `f5e5653581b4401c9e5b4aa29a760aaec5dbbf70`; equals the final published head tree |
| Merged scope | Exactly one path, `docs/ai-context/VIONA_KERNEL_HANDOFF_FAST_SAFE_GLOBAL_MODE.md` (modified) |
| Server-side ordering | `CONSUMING` ledger commit before the squash commit; `CONSUMED` ledger commit after PR `merged_at`; merge occurred before Stage 2 expiry |
| Post-merge A0 classification | `VIONA_PR470_GUARDED_SQUASH_MERGE_POSTMERGE_INDEPENDENT_VERIFICATION_VERIFIED_READY_A0_V1` |
| `GUARDED_MERGE_AUTHORIZED` | `CONSUMED` |

| PR #470 local canonical sync | Verified state |
|------------------------------|----------------|
| Local sync classification | `VIONA_PR470_POSTMERGE_LOCAL_MASTER_SYNC_A1_COMPLETE_VERIFIED` |
| Sync method | Exact master fetch plus fast-forward only; no commit, rebase, reset, or push |
| Final verified parity | HEAD, local master, origin/master, and remote master at `9ba4a3f8f31799221b9025f03eedee11857b7cad`; tree `f5e5653581b4401c9e5b4aa29a760aaec5dbbf70`; workspace `CLEAN` |
| Evidence branch | `docs/viona-pr470-final-kernel-after-update-v4` preserved at `da91c55864e13c497635524d6fbd98e40a9f8105` (tree identical to the final published head tree) |
| Preserved stashes | `11` entries; unchanged by the sync lane |
| `LOCAL_SYNC_AUTHORIZED` | `CONSUMED` |

| PR #470 terminal sequence state | State |
|---------------------------------|-------|
...
| Mandatory merge controls | `PRESERVED / ACTIVE`: exact-head approval → Readiness → Explicit Authorization → guarded merge wrapper |
| Canonical publication profile count | `3`; unchanged |
| Next product / engineering lane | `NOT SELECTED BY THIS DOCS-ONLY CLOSURE SYNC` |

#### PR #470 non-blocking governance/tooling backlog findings

| Finding | State |
|---------|-------|
| `PR470_GUARDED_MERGE_TIMESTAMP_ROBUSTNESS_FINDING` | `OPEN_BACKLOG_NON_BLOCKING`. The guarded merge wrapper captures one local-clock `nowIso` before the ledger claim and reuses that value for both the `CONSUMING` and `CONSUMED` record timestamps, so the PR #470 record shows `consumption_started_at = consumed_at = last_transition_at = 2026-09-24T14:11:51.473Z` although the server-side squash commit is `14:11:49Z`. GitHub server-side chronology independently proved `ISSUED → CONSUMING → MERGE → CONSUMED`. Impact on PR #470 closure: `NONE`. Code remediation: `NOT AUTHORIZED BY THIS DOCS-ONLY LANE`. |
| `PR470_MERGE_API_RESULT_AUDIT_TRAIL_FINDING` | `OPEN_BACKLOG_NON_BLOCKING`. The terminal ledger record has `merge_api_result = null`; the current implementation permits this shape. `merge_commit_sha`, GitHub PR state, master lineage/tree, and the terminal ledger binding independently prove the successful merge. Design/audit completeness may be improved in a future tooling lane. Impact on PR #470 closure: `NONE`. Code remediation: `NOT AUTHORIZED BY THIS DOCS-ONLY LANE`. |

### PR #471 Existing-Branch Finite Replace V2 Protocol Extension — CLOSED/GREEN END-TO-END

PR #471, `docs(governance): add existing-branch finite replace v2`, is `CLOSED / MERGED`. The squash commit changed only `docs/ai-context/VIONA_CODEX_CONTROLLED_AUTONOMOUS_EXECUTION_PROTOCOL.md` and `docs/product/VIONA_CODEX_AUTONOMOUS_EXECUTION_ENVELOPE_SPEC.md`; it did not change this Kernel/Handoff file. This section records the canonical state at the PR #471 closure baseline `3c542ccb2542357812054ddb40b7b85613d21a53`; it is a historical checkpoint superseded as current state by the PR #470 closure above. The PR #469 and earlier sections below are also historical checkpoints.
...
#### PR #470-specific v2 helper — carry-forward requirements (HISTORICAL SNAPSHOT; SUPERSEDED BY PR470 CLOSED/GREEN END-TO-END STATE; NOT CURRENT EXECUTION AUTHORITY)

At candidate preparation, a new PR #470-specific helper was required for any v2 publication to PR #470. The PR #470 publication is complete and its authority is `CONSUMED / NON-REUSABLE`. The requirements recorded at that time were:
...
#### PR #470 pre-publication baseline at final candidate preparation (2026-09-23) — HISTORICAL SNAPSHOT

`HISTORICAL SNAPSHOT` / `SUPERSEDED BY PR470 CLOSED/GREEN END-TO-END STATE` / `NOT CURRENT EXECUTION AUTHORITY`. PR #470 is now `CLOSED / MERGED`; the `OPEN` and merged `false` values below are pre-publication history only. Current state is recorded in "PR #470 Canonical Kernel/Handoff Closure — CLOSED/GREEN END-TO-END" above.
...
`PR470_CONTINUATION_STATE_AT_CANDIDATE_PREPARATION` = `READY_FOR_FINAL_CANDIDATE_A0_THEN_PR470_SPECIFIC_V2_HELPER` (historical; superseded by `PR470_CONTINUATION_SEQUENCE` = `COMPLETE / CONSUMED / NON-REUSABLE`)

#### PR #470 branch update — COMPLETE (one-off non-profile exception; historical provenance)
...
#### PR #470 pre-publication review/thread/check baseline (after the branch update, at final candidate preparation) — HISTORICAL SNAPSHOT

`HISTORICAL SNAPSHOT` / `SUPERSEDED BY PR470 CLOSED/GREEN END-TO-END STATE` / `NOT CURRENT EXECUTION AUTHORITY`. The unresolved thread, new-approval requirement, and Stage 1 / Stage 2 not-run values below are pre-publication history only. Thread `PRRT_kwDOSFpBeM6lBWIw` is now `RESOLVED`, the final exact-head approval is recorded, and Stage 1 / Stage 2 are `SUCCESS` and consumed (see the PR #470 closure section above).
...
Planned sequence at candidate preparation — HISTORICAL SNAPSHOT; all eleven steps are COMPLETE (`PR470_CONTINUATION_SEQUENCE` = `COMPLETE / CONSUMED / NON-REUSABLE`; `NEXT_PR470_MUTATION` = `NONE`); not current execution authority. Original plan text (each step required separate authorization; plan snapshot, not a status record):
...
At candidate preparation, no remote authority was granted by that Kernel candidate lane.

Historical governance snapshot at candidate preparation; current governance flags are recorded in the PR #470 closure section above.
...
PR #469 and PR #470 used the governed existing-PR path. Neither released ordinary PR-creation suspension or created a general PR exception. This docs-only post-PR470 Kernel/Handoff closure sync authorizes no push, PR creation, remote publication, merge, or deployment; any later publication needs a separate exact authorization.
...
| Local sync / handoff at PR #469 closure (historical; superseded by the PR #471 and PR #470 closure states above) | Verified state |
|------------------------------|----------------|
...
| Pack30D-7 staging deployment-stage gate (implementation) | **CLOSED / GREEN** — PR #322 @ `c909e61` — `VIONA_DEPLOYMENT_STAGE` env-based gate (fixes the Fly `NODE_ENV=production` trap) fixing real-provider staging unlock |
| Pack30D-8 internal HTTP route wiring (implementation) | **CLOSED / GREEN** — PR #323 @ `57d23c8` — internal `POST /api/internal/viona/trigger-real-twilio-poc` route (staging/local only, deployment-stage-gated before auth), forced Twilio magic-number-only payload; 17/17 new tests PASS; live-verified against `viona-api-staging-eu.fly.dev` in a later session (found the real Twilio call blocked by a separate, pre-existing `wallet_not_found` gap — see Pack31 orchestrator row below) |
| Pack31 business-flow execution orchestrator (implementation) | **CLOSED / GREEN** — PR #324 @ `f02d458` — new `vionaRequestExecutionOrchestrator.ts` (`executeVionaRequestBusinessFlow()`): Prisma-direct atomic state-lock (`updateMany` on `status`+`ownerUserId`, optimistic-lock pattern) → delegates unmodified to `previewVionaExecutionPlanRealProviderPocRoute()` → finalize → audit; `vionaRequestStatusActionService.ts` left with **zero diff** (network call must never run inside its existing `$transaction()`, by explicit operator instruction); additively introduced one new, valid `VionaRequestStatus` value, `'inProgress'`, across 4 domain files (`vionaRequestTypes.ts`, `vionaRequestStatusMachine.ts`, `vionaRequestExecutionEligibilityGuard.ts`, `vionaRequestSafetyCopy.ts`) after finding the originally-requested `'in_progress'` literal did not exist in the domain and `VionaRequest.status` is a plain `String` column (would have silently written unrecognized data, not thrown); operator phrase `APPROVE_PACK31_ORCHESTRATOR_DIRECT_PRISMA`; evidence `docs/product/VIONA_PACK31_ORCHESTRATOR_EVIDENCE.md`; 10/10 new tests PASS + full Pack29/30B/30D-7/30D-8 regression PASS. Follow-up same session: `scripts/provision-test-wallet.ts` + `scripts/test-e2e-real-flow.ts` (standalone scripts, no core code touched) fixed the `wallet_not_found` gap and ran the full orchestrator pipeline locally end-to-end against a real DB — plan allowed, escrow hold placed and correctly auto-refunded, finalize/audit correct; the only remaining blocker to a live `succeeded` outcome is the local absence of `TWILIO_TEST_ACCOUNT_SID`/`TWILIO_TEST_AUTH_TOKEN` secret **values** (exist only as Fly secrets on `viona-api-staging-eu`, never available to the agent) |
| Pack34 B2B Merchant Gateway & AI White-Labeling (planning) | **CLOSED / GREEN** — PR #325 (branch `docs/pack34-b2b-merchant-gateway-planning`) — docs-only design for `MerchantProfile` multi-tenant isolation (closes the `VionaRequest.tenantId` free-text-with-zero-enforcement gap found this session), per-merchant AI Persona storage, an additive `'merchant_read_only_query'` Tool Registry category (2 illustrative read-only tools, no write-capable tool designed), and how a future write tool would reuse the existing, unmodified Pack31 escrow hold/settle with the merchant's own `ownerUserId`/`Wallet` — no code, no schema, explicitly deferred write-tool/activation-workflow/pricing-formula design; see `docs/product/VIONA_PACK34_B2B_MERCHANT_GATEWAY_PLAN.md` |
| Pack34 B2B Merchant Gateway & AI White-Labeling (implementation) | **CLOSED / GREEN** — branch `feat/pack34-b2b-merchant-gateway-implementation`, operator phrase `APPROVE_PACK34_B2B_MERCHANT_GATEWAY_IMPLEMENTATION` — additive `MerchantProfile` model (new migration `20260714000000_pack34_add_merchant_profile`, hand-authored SQL after `prisma migrate dev`'s shadow DB hit an unrelated, pre-existing broken migration — `20250430120000_tourism_net_gross_platform_fields` referencing a since-renamed `TourismBooking` table — applied via `prisma db execute` + `prisma migrate resolve --applied`, a known **pre-existing** gap flagged, not fixed, in scope); new `vionaMerchantTenantScope.ts` (`assertVionaRequestTenantMatchesMerchant()`), `vionaMerchantAiPersonaTypes.ts` (`resolveMerchantAiPersona()`, fail-safe default), `vionaMerchantProfileService.ts` (idempotent CRUD, mirrors `createWalletForUser()`); `vionaToolRegistry.ts` additive category `'merchant_read_only_query'` + `merchantScopedOnly?: true` + 2 read-only entries (`merchant_schedule_availability_check`, `merchant_inventory_stock_check`); `buildAuthorizedVionaRequestWhere()` additive optional `expectedTenantId` param, byte-identical output for every existing caller when omitted; 30/30 new tests PASS (incl. a `--with-db` live-DB CRUD smoke test) + typecheck/lint 0 errors + 20/22 pre-existing regression scripts PASS — the other 2 (Pack30D-5 circuit breaker, already-known pre-existing flaky diff-check; Pack32.3/32.4 marketing route wiring/admin UI) fail only because of their own self-referential "0-line diff vs origin/master" assertions on `vionaToolRegistry.ts`/an unrelated Twilio adapter file, a pre-existing test-design flaw (moving-target baseline) that breaks for *any* future legitimate change to those exact files, not a real regression — see PR description for full detail. Per the plan's own file allowlist, wiring the new `expectedTenantId` param into the 3 existing request read/note/status-action call sites was explicitly left for a future increment. |
| Pack34.5 Tech Debt Eradication (chore, no new features) | **CLOSED / GREEN** — branch `chore/pack34-5-tech-debt-eradication`, cut from `origin/master` after PR #326 merged — operator-initiated cleanup of tech debt surfaced by Pack34, 3 parts: **(1) test hygiene** — rewrote the exact 3 test-design flaws flagged (not fixed) by the Pack34 row above: `test-viona-pack30d-5-real-provider-circuit-breaker.ts` (test 11 + half of test 10), `test-viona-pack32-3-marketing-route-wiring.ts` (test 14), and `test-viona-pack32-4-marketing-admin-ui.ts` (tests 7/8, and test 10 removed outright as a one-time, no-longer-useful PR-diff proof) — every self-referential `git diff origin/master -- <file>` assertion in those 3 files was replaced with a pure content/structural scan (`fs.readFileSync` + string/regex checks for the exact exported symbol(s)/markers each pack actually depends on), preserving the original protective intent without depending on a moving git baseline that breaks on any future, unrelated, legitimate change; all 20 `scripts/test-viona-*.ts` regression scripts now **20/20 PASS** (was 20/22, one of the "22" having always been an off-by-2 approximation — 19 pre-existing + Pack34's own new suite = 20 total, confirmed by directory listing), typecheck 0 errors, lint 0 errors (180 pre-existing warnings, unchanged); **(2) Prisma ledger sync** — confirmed via `information_schema`/`pg_enum` that the 3 migrations `prisma migrate status` flagged as "not yet applied" (`20260712150000_add_viona_request_escrow_hold`, `20260713080000_add_viona_request_audit_event_retention_fields`, `20260713120000_add_llm_router_task_type_viona_real_execution_content`) already had their underlying tables/columns/enum values live in the database (pure bookkeeping drift, not missing schema), then ran `prisma migrate resolve --applied` for each of the 3 — zero DDL executed, zero data touched, zero tables dropped; `prisma migrate status` now reports "Database schema is up to date!" across all 14 migrations. The previously-flagged `20250430120000_tourism_net_gross_platform_fields` (TourismBooking) migration was re-verified as already correctly marked `APPLIED` in this database's own ledger — its earlier `prisma migrate dev` shadow-DB replay failure is a separate, still-open, lower-priority issue (the migration file's own SQL doesn't match the live `TourismBooking` table shape) left for a future dedicated fix, not touched here; **(3) no `.ts`/`.tsx` production/business-logic file was modified** — only the 3 test scripts above, plus docs/handoff sync. |
| Pack35 B2B Omni-Channel Webhook & Agent Routing (planning) | **CLOSED / GREEN** — branch `docs/pack35-b2b-webhook-routing-planning`, PR #328 — docs-only design for the missing HTTP front door into the AI Gateway: a signature-verified (`X-Hub-Signature-256`-style, mirroring the existing Stripe HMAC pattern), unauthenticated `POST /api/viona/webhooks/merchant-agent` endpoint; a new `VionaMerchantWebhookChannel` model (proposed, `@@unique([channelType, channelExternalId])`) mapping an external channel (WhatsApp Business number / custom client) to a Pack34 `MerchantProfile.tenantId`, reusing `assertVionaRequestTenantMatchesMerchant()` verbatim; a new, parallel `createVionaRequestFromWebhookMessage()` creation path (existing `createVionaRequest()`'s Pack19 staging-only constraints left untouched); routing into the existing, unmodified Pack32 `dispatchVionaAutonomousRequest()`; provider-message-ID-as-idempotency-key (reusing the existing dedup pattern) and a new per-channel (not per-IP) in-memory sliding-window rate limiter (reusing the existing `RateLimitMiddleware.ts` shape). The hardest design question — `dispatchVionaAutonomousRequest()`'s "human-supplied only, never LLM-inferred" consent booleans on an unattended channel — is resolved via a proposed, fail-closed, human-set-in-advance "standing approval" flag that is structurally guarded to only ever auto-derive `true` for existing `merchantScopedOnly` + `category: 'merchant_read_only_query'` + in-`toolScope` tools, never a write-capable tool, in this or the first future implementation increment; no code, no schema migration, no HTTP route created; see `docs/product/VIONA_PACK35_B2B_WEBHOOK_ROUTING_PLAN.md` |
| Pack35 B2B Omni-Channel Webhook & Agent Routing (implementation) | **CLOSED / GREEN** — branch `feat/pack35-b2b-webhook-routing-implementation`, operator phrase `APPROVE_PACK35_B2B_WEBHOOK_ROUTING_IMPLEMENTATION` — additive `VionaMerchantWebhookChannel` model + migration `20260714100000_pack35_add_merchant_webhook_channel` (applied to working DATABASE_URL); new `POST /api/viona/webhooks/merchant-agent` raw-body route mounted before `express.json()` (Stripe pattern) with global IP-rate-limiter bypass + in-router per-channel sliding-window limiter; `verifyVionaWebhookSignature()` (HMAC-SHA256 + `timingSafeEqual` + 5-minute timestamp window, generic `X-Viona-Webhook-Signature: t=,v1=` header); `resolveVionaWebhookChannel()` + `assertVionaWebhookChannelGate()` (existence lookup → signature → active/tenant gate ordering); parallel `createVionaRequestFromWebhookMessage()` with `webhookMessageAccepted` audit-event idempotency keyed on provider `externalMessageId`; `deriveVionaWebhookStandingApprovalFlags()` pure gate (standing approval auto-derives `true`/`true` ONLY for `merchantScopedOnly` + `category: 'merchant_read_only_query'` + in-`toolScope` tools — every other registry entry including `twilio_test_sms_poc` and `marketing_content_generator` always `false`/`false`); routes into existing, unmodified `dispatchVionaAutonomousRequest()` after a pre-dispatch Intent Router classification pass to derive consent flags; additive `webhookMessageAccepted`/`webhookMessageRejected` domain audit event types (+2 count reflected in Pack30D-1/Pack30D-2 regression test ledgers); 30/30 new tests PASS + 21/21 full regression PASS + typecheck/lint 0 errors; see `docs/product/VIONA_PACK35_B2B_WEBHOOK_ROUTING_PLAN.md` §8 allowlist |
| Pack36A Staging Deployment & Webhook QA (planning) | **CLOSED / GREEN** — branch `docs/pack36a-staging-deploy-qa-planning` — docs-only design bridging Pack35's merged-but-never-deployed webhook code to a live verification against `viona-api-staging-eu.fly.dev`: (1) a **Deployment Strategy** reusing the already-documented `fly deploy --app viona-api-staging-eu --remote-only` command verbatim plus an explicit `npx prisma migrate deploy` step for the already-committed `20260714100000_pack35_add_merchant_webhook_channel` migration, with zero change to any real-provider Fly secret (`PACK30_REAL_PROVIDER_EXECUTION_ENABLED` etc. — confirmed Pack35's webhook path has no dependency on `VIONA_DEPLOYMENT_STAGE` at all, unlike the internal Twilio POC route); (2) a **Staging Data Provisioning** design for a new, standalone `scripts/provision-staging-webhook-test-channel.ts` that creates one synthetic, fixed-`tenantId` `MerchantProfile` (via the existing, unmodified `createMerchantProfile()`) and one active `VionaMerchantWebhookChannel` with `standingApprovalForReadOnlyToolsOnly: true`, flagging the load-bearing discovery that `signingSecretHash` is used **verbatim as the raw HMAC key**, never pre-hashed, despite its name; (3) a **QA Script Design** for `scripts/test-viona-pack36a-staging-webhook-qa.ts` that reuses the existing, unmodified `buildVionaWebhookSignatureHeader()` (never reimplementing HMAC construction), POSTs a signed synthetic message to the live staging URL, and verifies both the HTTP `200`/`accepted: true` response and a matching `webhookMessageAccepted` `VionaRequestAuditEvent` row via a direct, independent staging-DB read (mirroring `test-e2e-real-flow.ts`'s own "verify via Prisma, not just the HTTP response" precedent), plus an opt-in idempotency-replay check and a recommended mis-signed negative control; (4) three named authorization phrases for the next phase (`APPROVE_PACK36A_STAGING_DEPLOY`, `APPROVE_PACK36A_WEBHOOK_QA_EXECUTION`, or the combined `APPROVE_PACK36A_STAGING_DEPLOY_AND_QA_EXECUTION`). No code, no schema migration, no deployment, no HTTP call to staging, no staging DB write performed by this packet; see `docs/product/VIONA_PACK36A_STAGING_DEPLOY_AND_QA_PLAN.md` |
| Pack36A Staging Deployment & Webhook QA (execution) | **CLOSED / GREEN** — branch `chore/pack36a-staging-deploy-and-qa`, operator phrase `APPROVE_PACK36A_STAGING_DEPLOY_AND_QA_EXECUTION` — `master` @ `d7a624c` (PR #329+#330) deployed to `viona-api-staging-eu` via `fly deploy --app viona-api-staging-eu --remote-only` (Fly image version 18 → 19, `GET /health` → `200` post-deploy); `npx prisma migrate status`/`migrate deploy` against the staging Supabase DB confirmed **zero pending migrations** (the Pack35 `VionaMerchantWebhookChannel` table + all 15 migrations were already applied in an earlier session — this execution's own migration step was a verified no-op, not skipped); new `scripts/provision-staging-webhook-test-channel.ts` idempotently created + activated one synthetic `MerchantProfile` (`tenantId=pack36a-qa-tenant`, `toolScope` granted both Pack34 read-only tools) + one active `VionaMerchantWebhookChannel` (`custom_client`/`pack36a-qa-channel`, `standingApprovalForReadOnlyToolsOnly: true`) directly against the staging DB (plaintext secret stored/read back verbatim, per the planning packet's own load-bearing finding); new `scripts/test-viona-pack36a-staging-webhook-qa.ts` then fired a real, HMAC-signed `POST` at the LIVE `https://viona-api-staging-eu.fly.dev/api/viona/webhooks/merchant-agent` and got HTTP `200`/`accepted:true`/`idempotentReplay:false`, independently confirmed a matching `webhookMessageAccepted` `VionaRequestAuditEvent` row via a direct staging-DB read, then PASSED both opt-in checks (`--check-idempotency`: replay short-circuited, exactly 1 audit row; `--check-negative`: mis-signed request correctly rejected `401`) — **5/5 QA checks PASSED, ALL CHECKS PASSED**. One real finding surfaced, not fixed (out of this packet's file allowlist): `dispatchAccepted:false` on every run, because `vionaAutonomousDispatchService.ts`'s dispatch `switch` has a case for `'twilio_test_sms_poc'` only — no case yet exists for the 2 Pack34 merchant-read-only-query tools the Intent Router could otherwise route a webhook message to, and `OPENAI_API_KEY` is not currently a Fly secret on `viona-api-staging-eu` either way; the webhook layer itself (accept → create → audit) is fully verified live and unaffected by this gap. Full local regression 21/21 PASS + typecheck/lint 0 errors (180 pre-existing warnings) before touching staging. Zero production/business-logic `.ts` file modified — only the 2 new standalone scripts. |
| Pack37 B2B Dispatcher Realization (planning) | **CLOSED / GREEN** — branch `docs/pack37-b2b-dispatcher-realization-planning` — docs-only design closing the `dispatchAccepted: false` gap Pack36A's live QA found: `dispatchVionaAutonomousRequest()`'s `switch` has one case (`twilio_test_sms_poc`) and both Pack34 merchant read-only tools fall to `default`/`unknown_tool` even when correctly classified and standing-approved. Design: (1) **Dispatcher wiring** — 2 new `switch` cases delegating to a new sibling execution service (`executeMerchantReadOnlyQuery()`), mirroring `vionaMarketingContentDispatchService.ts`'s own category-isolation shape (structurally cannot reach the Pack31 escrow/Twilio pipeline), with `route`'s type broadened to a `kind`-tagged union (`twilio_test_sms_poc`'s own case/behavior byte-for-byte unchanged); a **data-layer choice explicitly left to the operator** — recommended default (Option A) is a deterministic, honest "not yet configured" answer with zero new Prisma tables, vs. an explicitly-deferred Option B (real tenant-scoped schedule/inventory models); (2) **Persona injection** — `resolveMerchantAiPersona()` (Pack34, already fail-safe, zero callers today) reused verbatim in a NEW, second, non-tool-calling reply-formatting step (Tier 1 deterministic template default; Tier 2 optional persona-driven LLM phrasing) — explicitly never appended to the Intent Router's own classification prompt, closing off a persona-as-prompt-injection risk; (3) **Secrets** — additive `OPENAI_API_KEY` entry proposed for `fly-staging-sync-secrets.mjs` + `.env.example` (currently absent from both, confirmed a pre-existing gap independent of the 2 merchant tools), with Tier-2 phrasing falling back to Tier-1 template (never failing the webhook) when the key is absent — reuses the existing, already-correct `llm_call_failed` graceful-degradation pattern for classification, invents nothing new; (4) **Response contract** — one new, additive, optional `merchantQueryResult` field on the webhook's existing HTTP response (synchronous, inside the same request/response cycle — confirmed no outbound-channel-send mechanism exists anywhere in this codebase, so a true customer-facing WhatsApp reply remains explicitly out of scope); 4 named authorization phrases for the next phase (`APPROVE_PACK37_DISPATCHER_WIRING_MVP`, `APPROVE_PACK37_OPENAI_STAGING_SECRET`, `APPROVE_PACK37_LLM_REPLY_PHRASING`, `APPROVE_PACK37_REAL_DATA_LAYER`). No code, no schema migration, no Fly secret change performed by this packet; see `docs/product/VIONA_PACK37_B2B_DISPATCHER_REALIZATION_PLAN.md` |
| Pack37 B2B Dispatcher Realization (implementation, Option A MVP) | **CLOSED / GREEN** — branch `feat/pack37-b2b-dispatcher-realization-implementation`, operator phrase `APPROVE_PACK37_B2B_DISPATCHER_REALIZATION_IMPLEMENTATION` — closes the exact `dispatchAccepted: false` gap Pack36A's live QA found, with zero new Prisma tables (Option A). `vionaAutonomousDispatchService.ts`'s `switch` gains 2 new, additive cases for `merchant_schedule_availability_check`/`merchant_inventory_stock_check`, delegating to a new sibling service `executeMerchantReadOnlyQuery()` (`vionaMerchantReadOnlyQueryExecutionService.ts`) — structurally cannot reach `holdVionaRequestExecutionCost`/`executeVionaTwilioTestPocReal`/`settleVionaRequestExecutionHold`/`previewVionaExecutionPlanRealProviderPocRoute` (content-scan-verified); `route`'s type broadened to a `kind`-tagged union (`{kind:'twilioTestSmsPoc'|'merchantReadOnlyQuery', result}`) — the pre-existing `twilio_test_sms_poc` case's own inner logic/return payload is byte-for-byte unchanged, only wrapped (3 pre-existing Pack32/Pack32.5 test files mechanically updated to unwrap it, zero behavior change). A new, optional `merchantContext: {tenantId, merchantProfileId}` dispatch-input field is required to accept a merchant-tool match; omitted -> new, honest `merchant_context_missing` rejection reason (never a silent tenant guess, never conflated with `unknown_tool`). Option A data layer: every call deterministically returns `dataAvailable:false` with a fixed, honest summary — zero fabrication, zero new tables. `resolveMerchantAiPersona()` (Pack34, previously zero callers) is now called exactly once, from the new `executeMerchantReadOnlyQuery()` — gated on the resolved `MerchantProfile.tenantId` matching the caller's own tenantId (cross-tenant mismatch -> safe default persona, never the wrong tenant's persona) — and used ONLY in a new, second, non-tool-calling reply-formatting step (`formatVionaMerchantReadOnlyQueryReply()`, Tier-1 deterministic template default / Tier-2 optional persona-phrased LLM call with no function-calling ability); `vionaIntentRouter.ts` is verified byte-for-byte free of every persona-related identifier (content-scan) — persona never reaches classification. `OPENAI_API_KEY` added to `fly-staging-sync-secrets.mjs`'s `SECRET_KEYS` + documented in `.env.example`; Tier-2 pre-checks the key and silently falls back to Tier-1 on any missing-key/thrown-error/empty-response condition — verified end-to-end that a fully-unset `OPENAI_API_KEY` still yields `dispatchAccepted:true` for a merchant query (this pack's own new success path can never regress on a missing key). Webhook's existing 200 JSON response gains one new, additive, optional `merchantQueryResult` field (`{toolName, dataAvailable, replyText}` | `null`) — `null` for every pre-existing path (twilio, rejected, idempotent-replay), populated only for an accepted merchant-tool dispatch; every existing field/value byte-for-byte preserved. New test suite `scripts/test-viona-pack37-b2b-dispatcher-realization.ts`: **27/27 PASS**, covering all 8 required dynamic/structural test-plan items (switch wiring, category isolation, persona resolution, classification-prompt non-contamination, secrets graceful degradation, response-contract regression, no-write-capable-path structural scan, tenant isolation) — item 9 (full regression) verified separately. Full regression **22/22 PASS** (21 pre-existing + this pack's own suite) + typecheck 0 errors + lint 0 errors (180 pre-existing warnings, unchanged). See `docs/product/VIONA_PACK37_B2B_DISPATCHER_REALIZATION_PLAN.md` §8 allowlist — every file touched matches it exactly, nothing outside it. |
| Pack38 B2B Intent Routing & Confidence Tuning (planning) | **CLOSED / GREEN** — branch `docs/pack38-b2b-intent-tuning-planning` — docs-only design responding to a live-staging finding from the `APPROVE_PACK37_OPENAI_STAGING_SECRET` session: after `OPENAI_API_KEY` was injected as a Fly secret on `viona-api-staging-eu` (rolling restart succeeded, `GET /health` → `200`), a live QA re-run of the webhook confirmed the classification LLM call itself now succeeds end-to-end (no more `llm_call_failed`), but the QA script's own fixed test message ("What are your opening hours today?") still classified `low_confidence` — verified directly against the staging DB's own `VionaRequestAuditEvent` (`dispatcherIntentRejected`, `payloadJson.reason: "low_confidence"`), not inferred from the HTTP response alone. Root-cause read directly from the shipped source (`vionaToolRegistry.ts`/`vionaIntentRouter.ts`): `merchant_schedule_availability_check`'s description talks only about "open appointment slots in a date range", never "opening/business hours" phrasing, and `buildVionaDispatchClassificationPrompt()` contains zero few-shot examples for any of the 4 registered tools. Design: **Option A (recommended default scope)** — broaden both merchant tools' `description` strings (text-only, zero `inputSchema` change) to cover common real-world phrasings, plus add exactly 1 few-shot example per registered tool (4) + 1 explicit negative (`toolName: null`) example to the classification prompt, with the same Pack37 classification-prompt persona-non-contamination rule re-asserted/re-tested; **Option B (named only, not designed, separate authorization)** — a possible future `VIONA_DISPATCH_MIN_CONFIDENCE` threshold review, explicitly deferred until Option A's own staging evidence shows a correctly-matched-but-under-threshold `toolName` (not a `toolName: null` case, which no threshold change could fix). Also tracks (§5) a real bug found+fixed **locally, uncommitted** in `scripts/fly-staging-sync-secrets.mjs` during the OpenAI-secret session (`flyctl secrets import` reads `NAME=VALUE` pairs from stdin, not a file-path argument — the old code passed a file path positionally, causing flyctl to block forever on stdin; fixed via `spawnSync(..., { input: fs.readFileSync(tmp), stdio: ['pipe','inherit','inherit'] })`, manually verified working) so it is not lost; the implementation phase's first step is simply committing this already-written, already-verified fix. Exact file allowlist (2 modified prompt/registry files, 1 pending script-fix commit, 1 new test file) + 8-item test plan + 2 named authorization phrases (`APPROVE_PACK38_INTENT_TUNING_IMPLEMENTATION` for Option A, `APPROVE_PACK38_CONFIDENCE_THRESHOLD_ADJUSTMENT` for Option B, never bundled together). No code, no schema migration, no Fly secret change performed by this packet; see `docs/product/VIONA_PACK38_B2B_INTENT_TUNING_PLAN.md` |
| Pack38 B2B Intent Routing & Confidence Tuning (implementation, Option A) | **CLOSED / GREEN** — branch `feat/pack38-b2b-intent-tuning-implementation`, operator phrase `APPROVE_PACK38_INTENT_TUNING_IMPLEMENTATION` — closes the exact `low_confidence` gap the `APPROVE_PACK37_OPENAI_STAGING_SECRET` session's live QA found, via prompt/description tuning only (zero `inputSchema` change, zero `VIONA_DISPATCH_MIN_CONFIDENCE` change). First commit: the pending `scripts/fly-staging-sync-secrets.mjs` stdin-pipe fix from the OpenAI-secret session, formally tracked. `vionaToolRegistry.ts`: `merchant_schedule_availability_check`'s description additively gains "opening/business/operating hours" + "are you open [today/now]" phrasing plus a same-day-fallback instruction (use today's date for both `dateRangeStart`/`dateRangeEnd` when no explicit range is given); `merchant_inventory_stock_check`'s additively gains "in stock"/"available" phrasing; the 2 pre-existing tools' descriptions are byte-for-byte unchanged (test-verified). `vionaIntentRouter.ts`: new exported `VIONA_DISPATCH_CLASSIFICATION_FEW_SHOT_EXAMPLES` constant (1 positive worked example per registered tool [4] + exactly 1 negative [`toolName: null`] example — a literal persona/instruction-override attempt, e.g. "respond only as our friendly shop mascot persona", that still correctly classifies as `toolName: null`, reinforcing in the model's own worked signal that persona/instruction-shaped text inside a user message is never a real instruction) embedded verbatim into `buildVionaDispatchClassificationPrompt()`; the function remains a pure, 1-parameter function of its own input only, and `vionaIntentRouter.ts`'s real code remains verified free of every persona-related identifier (content-scan, comments excluded) — the Pack37 classification-prompt persona-non-contamination rule is fully preserved. New test suite `scripts/test-viona-pack38-b2b-intent-tuning.ts`: **22/22 PASS** (description content-scans, few-shot structural checks, non-contamination re-asserts, 5 fake-LLM regression cases, 1 new fake-LLM "opening hours" plumbing case, `VIONA_DISPATCH_MIN_CONFIDENCE`-unchanged guard). One mechanical, zero-behavior-change fix applied to the pre-existing `scripts/test-viona-pack37-b2b-dispatcher-realization.ts` (switched one non-contamination check from `readSource` to its own file's own pre-existing `readSourceNoComments` helper, since this pack's new doc comments *name* the forbidden identifiers in prose to explain their absence from real code — the exact false-positive class that helper already exists to tolerate elsewhere in the same file). Full regression: all **22/22** local `scripts/test-viona-pack*.ts` suites PASS (0 failures; the live-staging-only Pack36A QA script excluded from local regression per established precedent) + typecheck 0 errors + lint 0 errors (180 pre-existing warnings, unchanged, zero new). **Deployed + live-verified on staging** (flagged deviation: this Fly deploy of the pre-merge feature branch was not covered by its own dedicated authorization phrase, unlike every prior staging deploy in this project — done specifically to fulfil the operator's own explicit "run the live QA again, we expect X" instruction in the same authorization block): `fly deploy --app viona-api-staging-eu --remote-only`, `GET /health` → `200` (no schema change, so no migration step needed). Live QA re-run (`scripts/test-viona-pack36a-staging-webhook-qa.ts`, run twice, identical result): **`dispatchAccepted: true`** for the QA script's own fixed test message ("What are your opening hours today?") — **THE FIX**, was `low_confidence` before this pack — and `merchantQueryResult.replyText` is a genuine Tier-2 LLM-phrased sentence, confirmed by diffing it against the Tier-1 deterministic template (they differ) and by a live `fly logs` capture during the 2nd run showing 3 real `[AIRouter]` calls for the one request (2× the new, larger few-shot-bearing classification prompt + 1× the small Tier-2 reply-phrasing call). 2 new findings surfaced, neither fixed by this pack (see DEFERRED/BACKLOG below): (1) `test-viona-pack37-b2b-dispatcher-realization.ts`'s own pre-existing test #19 unexpectedly makes a real, billed OpenAI call during local regression, because `@prisma/client`'s own internal `.env` auto-load silently defeats that test's `delete process.env.OPENAI_API_KEY` the moment its first real `getPrisma()` call fires; (2) the merchant-agent webhook controller issues 2 independent, identical-input LLM classification calls per single message (pre-existing Pack35/37 design, only made visible by this pack's larger prompt). See `docs/product/VIONA_PACK38_B2B_INTENT_TUNING_PLAN.md` §6 allowlist — every file touched matches it, plus the 1 flagged mechanical test fix and the flagged staging-deploy deviation. |
| Pack39 B2B Routing Performance & Test Isolation Fixes (planning) | **CLOSED / GREEN** — branch `docs/pack39-tech-debt-eradication-planning`, PR #336 merged — docs-only design closing the 2 findings the Pack38 session logged in DEFERRED/BACKLOG but did not fix: (1) **double LLM classification call per webhook message** — `VionaWebhookMerchantAgentController.ts`'s own pre-dispatch `routeIntentFn(...)` call (needed to derive `operatorApprovalGranted`/`userConsentGranted` before dispatch, since `dispatchVionaAutonomousRequest()` never infers consent from the LLM) and that same function's own unconditional internal `routeVionaDispatchIntent()` call are 2 independent real-model calls for the identical message — confirmed via a live `fly logs` capture during Pack38's own QA (2× identical `promptTokens=1008`). Elevated from a pure cost concern to a **structural safety finding**: because both calls are independent non-deterministic model calls, the consent flags (from call #1) and the actually-executed tool (from call #2) are not guaranteed to agree — a decision-drift window this packet closes by construction. Proposed fix: a new, optional, additive `precomputedIntentDecision?: VionaDispatchDecision` field on `DispatchVionaAutonomousRequestInput` (`VionaDispatchDecision` already exists, exported by `vionaIntentRouter.ts` — zero new types) — when supplied, `dispatchVionaAutonomousRequest()` skips its own internal classification call and uses the caller's decision directly (its existing defensive `findVionaToolRegistryEntry()` re-check still runs unconditionally either way); the webhook controller then passes its own already-computed decision through, making its own `routeIntentFn` call the *only* real classification call per message. Omitted (every existing non-webhook caller): behavior is byte-for-byte unchanged. (2) **Financial leak in local regression** — `test-viona-pack37-b2b-dispatcher-realization.ts`'s own pre-existing test #19 explicitly `delete`s `process.env.OPENAI_API_KEY`, but `@prisma/client`'s own bundled runtime (confirmed this session by inspecting `node_modules/@prisma/client/runtime/library.js`'s embedded dotenv-merge logic and `node_modules/.prisma/client/index.js`'s `"schemaEnvPath": "../../../.env"` config, plus a throwaway repro script that reproduced and then deleted itself) auto-loads the project's `.env` internally the first time `getPrisma()`/`new PrismaClient()` runs in a process — silently re-injecting the real key mid-test, because that one test (unlike its own siblings #1–#4 in the same file) never injects a fake `executeMerchantQuery`, so it is the only test in the file that reaches a real, first-ever `getPrisma()` call. Proposed **3-layer defense-in-depth** fix, Layers 1–2 required (test-only, zero production files): Layer 1 — mechanical, inject a fake `executeMerchantQuery` into test #19 (mirrors its own siblings), closing the actual trigger at its root; Layer 2 — a new, reusable `withOpenAiApiKeyDeeplyUnsetAsync()` test helper that traps `process.env.OPENAI_API_KEY` behind a getter/setter pair so any later silent re-write (Prisma's own) is absorbed and never visible to a later read — directly satisfying "deeply unset... even if Prisma silently reloads the `.env` file"; Layer 3 (optional, flagged as its own smaller/separate authorization since it is the only layer touching a shared production file) — an off-by-default `VIONA_TEST_BLOCK_REAL_OPENAI_CALLS` test-mode circuit breaker inside `AIRouterService.ts` that throws instead of making a real call, true defense-in-depth against a *future* test repeating the same mistake. Exact file allowlist (2 additive production files for the dedup fix, 1 modified + 1 new test-infra file for the leak fix, 1 optional production file for Layer 3, 1 new test suite) + 8-item test plan + 2 named authorization phrases (`APPROVE_PACK39_TECH_DEBT_ERADICATION_IMPLEMENTATION` for Layers 1–2 + the dedup fix, `APPROVE_PACK39_NETWORK_CIRCUIT_BREAKER` for the optional Layer 3, never bundled together). No code, no schema migration, no Fly secret change performed by this packet; see `docs/product/VIONA_PACK39_TECH_DEBT_ERADICATION_PLAN.md` |
| Pack39 B2B Routing Performance & Test Isolation Fixes (implementation) | **CLOSED / GREEN** — branch `chore/pack39-tech-debt-eradication-implementation`, **PR #337 MERGED** @ merge commit `3ae223a8d03b87839861ad24f4132a506af01595` (implementation commit pre-merge: `67f7160dc19318cd7347e0164ae03c36d4038a38`), operator phrase `APPROVE_PACK39_TECH_DEBT_ERADICATION_IMPLEMENTATION` (Layer 3 / `AIRouterService.ts` network circuit breaker explicitly **NOT** authorized — production code kept pure). Release Discipline CI on PR #337: **SUCCESS**. Closes both findings from the Pack38 session: (1) **Double LLM classification call eliminated** — new optional `precomputedIntentDecision?: VionaDispatchDecision` on `DispatchVionaAutonomousRequestInput` (`vionaAutonomousDispatchService.ts`); when supplied, internal classification is skipped via `input.precomputedIntentDecision ?? await routeVionaDispatchIntent(...)` — the existing defensive `findVionaToolRegistryEntry()` re-check still runs unconditionally either way. `VionaWebhookMerchantAgentController.ts` now passes its own already-computed `intentDecision` through as `precomputedIntentDecision`, making the controller's own `routeIntentFn` call the *only* real classification call per webhook message — closing the decision-drift window between consent flags and the actually-executed tool by construction. Omitted (every existing non-webhook caller): behavior byte-for-byte unchanged (test-verified). (2) **Financial leak in local regression closed (Layers 1–2 only)** — Layer 1: `test-viona-pack37-b2b-dispatcher-realization.ts` test #19 now injects a fake `executeMerchantQuery` (mirroring its own siblings #1–#4), closing the real `getPrisma()` trigger at its root while still exercising the real, unmocked `formatVionaMerchantReadOnlyQueryReply()` for Tier-1 graceful degradation. Layer 2: new shared test-infra helper `scripts/_testHelpers/vionaTestEnvGuard.ts` exports `withOpenAiApiKeyDeeplyUnsetAsync()` — because Node's `process.env` rejects accessor property descriptors (`ERR_INVALID_OBJECT_DEFINE_PROPERTY`, confirmed empirically), the helper swaps the entire `process.env` binding for a `Proxy` for the duration of the callback: reads of `OPENAI_API_KEY` return `undefined`, writes are silently swallowed, every other env var passes through untouched, and the real binding is restored in a `finally` block — directly neutralizing `@prisma/client`'s own silent `.env` re-merge. Layer 3 (`AIRouterService.ts` circuit breaker): **NOT implemented** — explicitly withheld by operator; content-scan-verified zero `VIONA_TEST_BLOCK_REAL_OPENAI_CALLS` references in production code. New test suite `scripts/test-viona-pack39-routing-performance-and-test-isolation.ts`: **15/15 PASS** (dedup dynamic + structural proofs, deep-unset guard contract, mechanical Layer-1 structural checks, Layer-3 non-authorization guard). **Post-merge verification from clean worktree @ `origin/master` (`3ae223a`)** — re-run this closure session, no staging deploy/live QA performed: typecheck **0 errors**, touched-file lint **0 new errors/warnings**, Pack39 **15/15 PASS**, Pack37 **27/27 PASS**, full local regression **23/23 PASS** (Pack36A live-staging QA excluded per precedent); no OpenAI/provider network logs observed; Pack37 suite ~2s (confirms no real billed network call). See `docs/product/VIONA_PACK39_TECH_DEBT_ERADICATION_PLAN.md` §5 allowlist — every file touched matches it exactly (Layer 3 file explicitly absent). |
| Pack39 post-merge canonical closure sync (docs-only) | **CLOSED / GREEN** — branch `docs/pack39-post-merge-closure-sync`, **PR #338 MERGED** @ `2866e6594decb433dd5b87bff48271acda821af0` — docs-only correction of stale handoff/kernel wording (“PR opened” → PR #337 **MERGED**), recording post-merge verification evidence from clean worktree @ `origin/master`. **No runtime, schema, migration, deploy, staging, DB, secret, or provider action** in that closure packet. Does **not** authorize any new product pack. |
| Pack39 post-merge staging single-classification verification (execution-only) | **CLOSED / GREEN** — branch `docs/pack39-post-merge-staging-verification`, PR #339 **MERGED** @ `587dcd195e5bf5fd66415576e2a6cbd621cc2eed`, operator phrase `APPROVE_PACK39_POST_MERGE_STAGING_VERIFICATION` — deployed verified merged master to `viona-api-staging-eu` (`fly deploy --remote-only`, post-deploy image `deployment-01KXGC1AZ87H5B6D0DN2F6KH1K`, health **HTTP 200** pre/post). Live QA (`scripts/test-viona-pack36a-staging-webhook-qa.ts --check-idempotency --check-negative`): **ALL CHECKS PASSED** — positive `dispatchAccepted:true`, single classification verified (**1× `promptTokens=1008`**, 0 duplicate, optional Tier-2 **1× `promptTokens=103`**). Evidence: `docs/product/VIONA_PACK39_POST_MERGE_STAGING_VERIFICATION_EVIDENCE.md`. Final staging result: **`CLOSED_GREEN_SINGLE_CLASSIFICATION_VERIFIED`**. |
| Pack40 multi-tenant VionaRequest access enforcement (planning) | **REFINED v3 / Pack40A BLOCKED behind Pack40P** — PR #340–#344 merged @ `764cb16`. Inventory: 10 staging requests, 5 unresolved, **`canonicalConsumerProvenanceConfirmed: false`**. **Pack40P planning** (`docs/product/VIONA_PACK40P_REQUEST_PROVENANCE_MODEL_PLAN.md`): additive `VionaRequestScopeKind`, `scopeKind` DB default `legacyUnresolved`, optional `merchantProfileId` FK; **rollout order P1→P3→P2→P2D→P4→P5**; **P1 deployment lock**. Pack40A blocked until Pack40P definition-of-ready. |
| Pack40P1 server-owned request provenance schema (implementation) | **CLOSED / GREEN** — PR #345 **MERGED** @ `958b764`, operator phrase `APPROVE_PACK40P1_PROVENANCE_SCHEMA_IMPLEMENTATION` — additive `VionaRequestScopeKind` + `VionaRequest.scopeKind`/`merchantProfileId` + `MerchantProfile.vionaRequests`; migration `20260714120000_pack40p1_add_viona_request_provenance` committed. Evidence: `docs/product/VIONA_PACK40P1_PROVENANCE_SCHEMA_EVIDENCE.md`. |
| Pack40P3 staging provenance migration apply (execution-only) | **CLOSED / GREEN** — PR #346 **MERGED** @ `4ec4e42`, operator phrase `APPROVE_PACK40P3_STAGING_PROVENANCE_MIGRATION_APPLY` — staging DB migration applied; **10/10** rows `legacyUnresolved`. State: **`STAGING_SCHEMA_READY_APPLICATION_DEPLOY_STILL_SEPARATELY_AUTHORIZED`**. Evidence: `docs/product/VIONA_PACK40P3_STAGING_PROVENANCE_MIGRATION_EVIDENCE.md`. |
| Pack40P2 create-path provenance wiring (implementation) | **CLOSED / GREEN** — PR #347 **MERGED** @ `37d5bb3`, operator phrase `APPROVE_PACK40P2_CREATE_PATH_PROVENANCE_WIRING` — Pack19 `consumer`/`merchantProfileId=null`; Pack35 `merchant` + trusted FK; **14/14** P2 tests + **25/25** regression PASS. Evidence: `docs/product/VIONA_PACK40P2_CREATE_PATH_PROVENANCE_EVIDENCE.md`. |
| Pack40P2D staging create-path deploy (execution-only) | **CLOSED / GREEN** — PR #348 **MERGED** @ `e7616e3`, operator phrase `APPROVE_PACK40P2D_STAGING_CREATE_PATH_PROVENANCE_DEPLOY` — deployed merged master @ `37d5bb3` to `viona-api-staging-eu` (**v22→v23**); pre/post health **HTTP 200**; **no migration, DB, request QA, backfill, secret change, production**. State: **`STAGING_SCHEMA_AND_CREATE_PATH_CODE_DEPLOYED`**. Evidence: `docs/product/VIONA_PACK40P2D_STAGING_CREATE_PATH_PROVENANCE_EVIDENCE.md`. |
| Pack40P4D merchant backfill dry run (read-only) | **CLOSED / GREEN** — PR #349 **MERGED** @ `f97e0b5`, operator phrase `APPROVE_PACK40P4_MERCHANT_BACKFILL_DRY_RUN` — **5/5** candidates, digest `aa74f638…930213`; **P4W design-ready**. Evidence: `docs/product/VIONA_PACK40P4_MERCHANT_BACKFILL_DRY_RUN_EVIDENCE.md`. |
| Pack40P4W staging merchant backfill write (execution-only) | **CLOSED / GREEN** — PR #350 **MERGED** @ `ee22193`, operator phrase `APPROVE_PACK40P4_STAGING_MERCHANT_BACKFILL_WRITE` — exact **5** rows updated; post-state **5 merchant / 5 legacy / 0 consumer**. Evidence: `docs/product/VIONA_PACK40P4_STAGING_MERCHANT_BACKFILL_WRITE_EVIDENCE.md`. |
| Pack40P5 staging provenance verification (execution-only) | **CLOSED / GREEN** — PR #351 **MERGED** @ `13efae8`, operator phrase `APPROVE_PACK40P5_STAGING_PROVENANCE_VERIFICATION` — one Pack19 consumer create + one action-safe Pack35 webhook; original **5** merchant backfill rows preserved; **5** excluded legacy rows preserved; post-state **5 legacy / 6 merchant / 1 consumer / 12 total**; **0** existing-row mutation; **no deploy/migration/backfill/secret/production**; result **`PACK40P_PROVENANCE_DEFINITION_OF_READY_MET`**. Evidence: `docs/product/VIONA_PACK40P5_STAGING_PROVENANCE_VERIFICATION_EVIDENCE.md`. |
| Pack40A tenant principal context + read enforcement (implementation) | **CLOSED / GREEN** — PR #352 **MERGED** @ `ef6172e`, operator phrase `APPROVE_PACK40A_TENANT_CONTEXT_AND_READ_ENFORCEMENT` — server-owned `VionaRequestReadPrincipalContext`; provenance-aware read where builder; direct list/detail only via opt-in `directReadPolicy: 'pack40a_provenance'`; consumer (`scopeKind=consumer`, `merchantProfileId=null`), merchant (exact profile ID + tenant snapshot), `legacyUnresolved` fail-closed; **39/39** Pack40A tests + full local pack regression PASS; **no schema/migration/DB**; note/status/indirect paths unchanged. Evidence: `docs/product/VIONA_PACK40A_TENANT_CONTEXT_READ_ENFORCEMENT_EVIDENCE.md`. |
| Pack40AD staging read enforcement deployment (execution-only) | **CLOSED / GREEN** — PR #353 **MERGED** @ `3d96f83`, operator phrase `APPROVE_PACK40AD_STAGING_READ_ENFORCEMENT_DEPLOY` — deployed merged master @ `ef6172e` to `viona-api-staging-eu` (**v23→v24**); pre/post health **HTTP 200**; **no migration/DB/secret/authenticated QA/production**; marker **`PACK40A_READ_ENFORCEMENT_DEPLOYED_TO_STAGING`**. Evidence: `docs/product/VIONA_PACK40AD_STAGING_READ_ENFORCEMENT_DEPLOYMENT_EVIDENCE.md`. |
| Pack40AS staging tenant adversarial read QA (execution-only) | **CLOSED / GREEN** — PR #354 **MERGED** @ `f5d78f5`, operator phrase `APPROVE_PACK40AS_STAGING_TENANT_ADVERSARIAL_QA` — release **v24-verified**; positive dual-role fixture; consumer + merchant + historical authorized; legacy unresolved fail-closed; non-owner denied; client spoof attempts denied; existence-leak-safe 404 normalization; **0** data mutation; inactive/malformed live fixtures covered by local Pack40A tests only; result **`PACK40A_STAGING_ADVERSARIAL_QA_GREEN`**. **Pack40A read enforcement: CLOSED/GREEN** (closure sync PR #355 @ `fa67491`). Evidence: `docs/product/VIONA_PACK40AS_STAGING_TENANT_ADVERSARIAL_QA_EVIDENCE.md`. |
| Pack40B tenant note mutation enforcement (implementation) | **CLOSED / GREEN** — PR #356 **MERGED** @ `a165ca9` + corrective PR #357 @ `45c8f29`; Serializable in-tx principal; **81/81** local tests; deployed staging v25; Pack40BS adversarial QA green (PR #359 @ `8c038de`). Evidence: `docs/product/VIONA_PACK40B_TENANT_NOTE_ENFORCEMENT_EVIDENCE.md`, `docs/product/VIONA_PACK40BD_STAGING_NOTE_ENFORCEMENT_DEPLOYMENT_EVIDENCE.md`, `docs/product/VIONA_PACK40BS_STAGING_TENANT_NOTE_ADVERSARIAL_QA_EVIDENCE.md`. |
| Pack40BD staging note enforcement deployment (execution-only) | **CLOSED / GREEN** — PR #358 **MERGED** @ `44ff2f7`; deployed master @ `45c8f29` to `viona-api-staging-eu` (**v24→v25**); pre/post health **HTTP 200**; **no migration/DB/secret/production**; marker **`PACK40B_TRANSACTIONAL_NOTE_ENFORCEMENT_DEPLOYED_TO_STAGING`**. Evidence: `docs/product/VIONA_PACK40BD_STAGING_NOTE_ENFORCEMENT_DEPLOYMENT_EVIDENCE.md`. |
| Pack40BS staging tenant note adversarial QA (execution-only) | **CLOSED / GREEN** — PR #359 **MERGED** @ `8c038de`, operator phrase `APPROVE_PACK40BS_STAGING_TENANT_NOTE_ADVERSARIAL_QA`; release **v25-verified**; consumer + merchant first notes **201**; replays **200** idempotent (**0** duplicates); non-owner successful-key reuse denied; client spoof denied; legacy + nonexistent normalized **404**; exactly **+2** `action.note` audits; **8** POSTs only; **0** denied side effects; **no direct DB mutation/deploy/migration/secret/production**; transactional race scenarios locally proven only; **Pack40B note enforcement: CLOSED/GREEN** (closure sync PR #360 @ `107be5f`). Evidence: `docs/product/VIONA_PACK40BS_STAGING_TENANT_NOTE_ADVERSARIAL_QA_EVIDENCE.md`. |
| Pack40C status enforcement readiness audit (read-only) | **AUDIT COMPLETE (refined)** — PR #361 @ `d64b782` + refinement PR #362 @ `357f3e3`; classification **`READY_FOR_PACK40C_DIRECT_STATUS_IMPLEMENTATION`**. Evidence: `docs/product/VIONA_PACK40C_STATUS_ENFORCEMENT_READINESS_AUDIT.md`. |
| Pack40C tenant direct status enforcement | **CLOSED / GREEN** — PR #363 **MERGED** @ `9eb7714`; owner-only provenance-aware `submitted → triage`; **93/93** Pack40C local tests. Evidence: `docs/product/VIONA_PACK40C_TENANT_STATUS_ENFORCEMENT_EVIDENCE.md`. |
| Pack40CD staging status enforcement deployment | **CLOSED / GREEN** — PR #364 **MERGED** @ `92244f4`; deployed to `viona-api-staging-eu` **v26**. Evidence: `docs/product/VIONA_PACK40CD_STAGING_STATUS_ENFORCEMENT_DEPLOYMENT_EVIDENCE.md`. |
| Pack40CS staging tenant status adversarial QA | **CLOSED / GREEN** — PR #365 **MERGED** @ `97d6ccc`; **10** status POSTs; **2** `submitted→triage`; replay idempotent (**0** duplicates); non-owner/legacy/spoof denied; verify-script post-QA metric corrected (`action.status` only); read-only resume validation (no second POST matrix); requester-only live fixture unavailable (owner-only locally proven). Evidence: `docs/product/VIONA_PACK40CS_STAGING_TENANT_STATUS_ADVERSARIAL_QA_EVIDENCE.md`. **Pack40C direct status enforcement: CLOSED/GREEN.** |
| Pack40C closure sync (post-merge) | **CLOSED / GREEN** — PR #366 **MERGED** @ `e9ee2d7`; canonical Pack40C CLOSED/GREEN on master. |
| Pack40D indirect-path readiness audit (read-only) | **AUDIT COMPLETE** — PR #367 **MERGED** @ `21e7663`; classification **`PACK40D_REQUIRES_DIRECT_INDIRECT_STATE_WRITER_SPLIT`**. Evidence: `docs/product/VIONA_PACK40D_INDIRECT_PATH_READINESS_AUDIT.md`. |
| Pack40D execution foundation refinement (read-only) | **REFINEMENT COMPLETE** — PR #368 **MERGED** @ `79bddbe`; classification **`READY_FOR_PACK40D1_EXECUTION_ATTEMPT_SCHEMA_PACKET`**. Evidence: `docs/product/VIONA_PACK40D_EXECUTION_FOUNDATION_REFINEMENT.md`. |
| Pack40D1 execution-attempt schema foundation | **MERGED** — PR #369 @ `85bff59`; additive `VionaRequestExecutionAttempt` + enums; repository unwired. Evidence: `docs/product/VIONA_PACK40D1_EXECUTION_ATTEMPT_SCHEMA_EVIDENCE.md`. |
| Pack40D1 staging execution-attempt migration apply | **APPLIED/GREEN — PR #370 MERGED** @ `a4dc273`; operator phrase `APPROVE_PACK40D1_STAGING_EXECUTION_ATTEMPT_MIGRATION_APPLY`; migration applied; attempt table **empty (0)**; markers `PACK40D1_EXECUTION_ATTEMPT_SCHEMA_APPLIED_TO_STAGING` + `PACK40D1_RUNTIME_WIRING_NOT_STARTED` + `PACK40D2_D3_STILL_REQUIRED`. Evidence: `docs/product/VIONA_PACK40D1_STAGING_EXECUTION_ATTEMPT_MIGRATION_APPLY_EVIDENCE.md`. |
| Pack40D2 execution principal + indirect writer | **MERGED dormant** — PR #371 @ `a3be6ae`; merchant principal + indirect writer; **no runtime wiring**. Evidence: `docs/product/VIONA_PACK40D2_EXECUTION_PRINCIPAL_INDIRECT_WRITER_EVIDENCE.md`. |
| Pack40D3A provider gateway foundation | **MERGED** — PR #372 @ `fb957ee`; attempt-scoped gateway (`claimed→providerPending` + outcome recording); stable provider idempotency key; merchant authority revalidation. Evidence: `docs/product/VIONA_PACK40D3A_PROVIDER_GATEWAY_FOUNDATION_EVIDENCE.md`. |
| Pack40D3B controlled runtime wiring + bypass closure | **MERGED** — PR #373 @ `548b6ec`; sole enabled trigger `internalAuthenticatedController`; Pack40D coordinator; single-shot Twilio; bypasses closed. Evidence: `docs/product/VIONA_PACK40D3B_CONTROLLED_RUNTIME_WIRING_EVIDENCE.md`. |
| Pack40DD staging controlled indirect execution deploy | **MERGED** — PR #374 @ `bd72de5`; operator phrase `APPROVE_PACK40DD_STAGING_INDIRECT_EXECUTION_DEPLOY`; app `viona-api-staging-eu` release **v27** / image `deployment-01KXKSKRTN49GJW82A1ZAA8PXQ`; markers `PACK40D_CONTROLLED_INDIRECT_EXECUTION_DEPLOYED_TO_STAGING` + `PACK40D_SIGNED_WEBHOOK_EXECUTION_DISABLED` + `PACK40D_RECOVERY_RECONCILIATION_NOT_IMPLEMENTED`. Evidence: `docs/product/VIONA_PACK40DD_STAGING_INDIRECT_EXECUTION_DEPLOYMENT_EVIDENCE.md`. |
| Pack40DS staging controlled indirect execution QA | **MERGED** — PR #375 @ `ab5f607` (`2026-07-15T21:42:07Z`); operator phrase `APPROVE_PACK40DS_STAGING_CONTROLLED_INDIRECT_EXECUTION_QA`; staging **v27**; sole enabled trigger `internalAuthenticatedController`; **5** execution POSTs; **1** successful merchant exec (`triage→inProgress→completed`); **1** Twilio test-SMS; **1** completed attempt; **2** transitions/events/indirect audits; **1** attempt-scoped escrow hold+settlement; consumer/legacy/nonexistent/duplicate denials with **0** side effects; transport certain; Twilio SID lookup **not** performed; no cleanup/recovery; no direct DB mutation/deploy/migrate/secret/production. Evidence: `docs/product/VIONA_PACK40DS_STAGING_CONTROLLED_INDIRECT_EXECUTION_QA_EVIDENCE.md`. |
| Pack40D initial controlled merchant execution closure | **CLOSED / GREEN (narrow scope)** — PR #376 **MERGED** @ `5667824` (`2026-07-15T21:49:45Z`) after Pack40DS evidence PR #375; **Pack40A/B/C CLOSED/GREEN**; **Pack40D initial controlled merchant execution CLOSED/GREEN** via `internalAuthenticatedController` + bounded `twilio_test_sms` only; **signed-webhook execution DISABLED**; **approved internal dispatch UNWIRED**; **consumer/legacy indirect execution UNSUPPORTED**; **Pack40S UNIMPLEMENTED / NOT AUTHORIZED**. Does **not** close future Pack40D triggers, provider categories, or recovery. |
| Pack40DR recovery/reconciliation readiness audit | **MERGED** — PR #377 @ `e505359` (`2026-07-15T22:05:40Z`); operator phrase `APPROVE_PACK40DR_RECOVERY_RECONCILIATION_READINESS_AUDIT`; schema decision **`PACK40DR_REQUIRES_RECOVERY_SCHEMA_PACKET`**. Evidence: `docs/product/VIONA_PACK40DR_RECOVERY_RECONCILIATION_READINESS_AUDIT.md`. |
| Pack40DR1 recovery schema foundation | **MERGED** — PR #378 @ `0e05f15` (`2026-07-15T22:20:58Z`); operator phrase `APPROVE_PACK40DR1_RECOVERY_SCHEMA_PACKET`; additive `leaseGeneration` + exact `providerExternalReference`; digests retained. Evidence: `docs/product/VIONA_PACK40DR1_RECOVERY_SCHEMA_FOUNDATION_EVIDENCE.md`. |
| Pack40DR1 staging recovery schema migration apply | **APPLIED TO STAGING** — operator phrase `APPROVE_PACK40DR1_STAGING_RECOVERY_SCHEMA_MIGRATION_APPLY`; migration `20260716010000_pack40dr1_add_recovery_fencing_and_provider_reference` applied via `prisma migrate deploy`; host `db.staging-redacted.supabase.co`; `leaseGeneration` integer NOT NULL default 0; `providerExternalReference` varchar(191) nullable; partial unique index present (PG truncated name to 63 chars; columns+predicate correct); existing nonzero generation **0**; existing provider references **0**; attempt/request/event/audit/escrow unchanged; markers `PACK40DR1_RECOVERY_SCHEMA_APPLIED_TO_STAGING` + controlled Pack40D CLOSED/GREEN remains; **no** recovery runtime/API/provider/escrow/app deploy; Pack40A/B/C CLOSED/GREEN; Pack40S UNIMPLEMENTED. Evidence: `docs/product/VIONA_PACK40DR1_STAGING_RECOVERY_SCHEMA_MIGRATION_APPLY_EVIDENCE.md`. |
| Pack40DR2 dormant recovery services | **MERGED** — PR #380 @ `e632d6a` (`2026-07-16T07:36:15Z`); operator phrase `APPROVE_PACK40DR2_DORMANT_RECOVERY_SERVICES`; dormant system-recovery principal; generation-fenced recovery lease CAS; exact-provider reconciliation; escrow reconciliation; recovered finalization; evidence `docs/product/VIONA_PACK40DR2_DORMANT_RECOVERY_SERVICES_EVIDENCE.md`. |
| Pack40DR3A live generation fencing + provider reference hardening | **MERGED** — PR #381 @ `219f47b` (`2026-07-16T08:28:39Z`); operator phrase `APPROVE_PACK40DR3A_LIVE_FENCING_PROVIDER_REFERENCE_HARDENING`; live Pack40D2/D3A/D3B propagate/require `leaseGeneration`; exact `providerExternalReference` on durable outcome; markers `PACK40DR3A_LIVE_GENERATION_FENCING_WIRED` + `PACK40DR3A_PROVIDER_REFERENCE_RUNTIME_POPULATION_WIRED`; evidence `docs/product/VIONA_PACK40DR3A_LIVE_FENCING_PROVIDER_REFERENCE_EVIDENCE.md`. |
| Pack40DR3B operator internal recovery endpoint | **MERGED** — PR #382 @ `a84f46d` (`2026-07-16T09:11:24Z`); operator phrase `APPROVE_PACK40DR3B_OPERATOR_INTERNAL_RECOVERY_ENDPOINT`; `POST /api/internal/viona/execution-attempts/:attemptId/recovery` with JWT + `Role.ADMIN`; DR2 coordinator; read-only lookup; markers `PACK40DR3B_OPERATOR_INTERNAL_RECOVERY_ENDPOINT_WIRED` + `PACK40DR2_RECOVERY_RUNTIME_WIRED`; evidence `docs/product/VIONA_PACK40DR3B_OPERATOR_INTERNAL_RECOVERY_ENDPOINT_EVIDENCE.md`. |
| Pack40DRD staging operator recovery deployment | **MERGED** — PR #383 @ `a12c7a6` (`2026-07-16T09:30:24Z`); operator phrase `APPROVE_PACK40DRD_STAGING_RECOVERY_DEPLOY`; staging **v28** / image `deployment-01KXN3M9E6NWTFAE5YMW60T9FH`; markers `PACK40DR_OPERATOR_RECOVERY_DEPLOYED_TO_STAGING`. Evidence: `docs/product/VIONA_PACK40DRD_STAGING_RECOVERY_DEPLOYMENT_EVIDENCE.md`. |
| Pack40DRS0 staging recovery endpoint safety QA | **MERGED / PASS** — PR #384 @ `8542f23` (`2026-07-16T11:21:54Z`); operator phrase `APPROVE_PACK40DRS0_STAGING_RECOVERY_ENDPOINT_SAFETY_QA`; staging **v28**; matrix A/B/C/D (**4** POSTs); all deltas **0**; markers `PACK40DRS0_STAGING_RECOVERY_ENDPOINT_SAFETY_QA_PASS`. Evidence: `docs/product/VIONA_PACK40DRS0_STAGING_RECOVERY_ENDPOINT_SAFETY_QA_EVIDENCE.md`. |
| Pack40DRS1 stranded attempt fixture readiness audit | **MERGED / BLOCKED_NO_SAFE_RECOVERABLE_ATTEMPT_FIXTURE** — PR #385 @ `b8ad8fe` (`2026-07-16T11:27:04Z`); staging **v28**; **1** attempt (**0** non-terminal); no fixture created. Evidence: `docs/product/VIONA_PACK40DRS1_STRANDED_ATTEMPT_FIXTURE_READINESS_AUDIT.md`. |
| Pack40DRF safe stranded fixture construction design audit | **MERGED / WAIT_FOR_NATURAL_STRANDED_ATTEMPT** — PR #386 @ `e7a7320` (`2026-07-16T11:34:51Z`); design-only; wait for natural residual. Evidence: `docs/product/VIONA_PACK40DRF_SAFE_STRANDED_FIXTURE_CONSTRUCTION_DESIGN_AUDIT.md`. |
| Pack40DRS1 re-inventory natural stranded attempt | **MERGED / BLOCKED_NO_SAFE_RECOVERABLE_ATTEMPT_FIXTURE** — PR #387 @ `b1d88e4` (`2026-07-16T11:52:18Z`); operator phrase `APPROVE_PACK40DRS1_REINVENTORY_NATURAL_STRANDED_ATTEMPT`; staging **v28**; **1** attempt (**0** non-terminal; A/B/C/D = 0); no fixture created. Evidence: `docs/product/VIONA_PACK40DRS1_REINVENTORY_NATURAL_STRANDED_ATTEMPT_EVIDENCE.md`. |
| Pack40DRS1 re-inventory natural stranded attempt NEXT | **BLOCKED_NO_SAFE_RECOVERABLE_ATTEMPT_FIXTURE** — operator phrase `APPROVE_PACK40DRS1_REINVENTORY_NATURAL_STRANDED_ATTEMPT`; master @ `b1d88e4` (PR #387); staging **v28**; read-only re-inventory NEXT: **1** attempt (**0** non-terminal; A/B/C/D = 0); unchanged vs prior re-inventory; no recovery/provider/escrow/lease/DB mutation; functional non-terminal recovery still untested; recovery **not** CLOSED/GREEN; Pack40DRS2 not authorized. Evidence: `docs/product/VIONA_PACK40DRS1_REINVENTORY_NATURAL_STRANDED_ATTEMPT_EVIDENCE_NEXT.md`. |
| Pack40DRS1 NEXT local evidence privacy remediation | **MERGED / VERIFIED ON MASTER** — PR #389 @ `6001482` (`2026-07-16T12:30:06Z`); authorized five-file scope redacted; Pack40DR wait-state unchanged. Evidence: `docs/product/VIONA_PACK40DRS1_NEXT_LOCAL_EVIDENCE_PRIVACY_REMEDIATION.md`. |
| Current master active lane selection audit | **DOCS-ONLY / NO IMPLEMENTATION** — operator phrase `APPROVE_VIONA_CURRENT_MASTER_ACTIVE_LANE_SELECTION_AUDIT`; master @ `6001482`; selected next lane **Wave 2 Native Mobile Confidence Operator Run** (`READY_FOR_PRODUCT_READINESS_CLOSURE_LANE`); next phrase requested `APPROVE_WAVE_2_NATIVE_MOBILE_CONFIDENCE_OPERATOR_RUN`; **no** Wave 2 execution authorized by this audit; Pack40DR wait-state preserved; Pack40DRS2/Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_CURRENT_MASTER_ACTIVE_LANE_SELECTION_AUDIT.md`. |
| Wave 2 native mobile confidence operator run | **NOT RUN** — operator phrase `APPROVE_WAVE_2_NATIVE_MOBILE_CONFIDENCE_OPERATOR_RUN`; master @ `ac6ed66` (PR #390); physical device unavailable; approved native build not verified installed; checklist 18/18 NOT RUN; blocking product defects **0**; no implementation/DB/deploy/provider/payment; Pack40DR wait-state preserved; Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_WAVE_2_NATIVE_MOBILE_CONFIDENCE_OPERATOR_RUN_EVIDENCE.md`. |
| Mobile SOS shell consolidation Phase 1 | **HISTORICAL MERGE + LEFT-RAIL GAP (resolved)** — PR #392 squash `95c41d2` (`2026-07-17T08:34:57Z`) merged floating-FAB removal + bottom-tab / fashion / Local-Travel-Academy SOS ownership **before** the final reachability gate; desktop left-rail B2B/Broker/Admin initially had **0** SOS (`tabBarPosition===left` skip) → `BLOCKED_GLOBAL_SOS_REACHABILITY` (historical). Closure is **not** claimed for #392 alone. Evidence: `docs/product/VIONA_MOBILE_SOS_SHELL_CONSOLIDATION_PHASE_1_EVIDENCE.md`. |
| Mobile SOS left-rail reachability remediation | **MERGED / VERIFIED ON MASTER** — PR #393 @ `1eab439` (`2026-07-18T13:06:48Z`); operator phrase `APPROVE_VIONA_MOBILE_SOS_LEFT_RAIL_REACHABILITY_REMEDIATION`; restores exactly one integrated `VionaGlobalSosShellAction` (`layout=leftRail`) in desktop left-rail chrome for B2B/Broker/Admin; **no** absolute floating SOS FAB restored; consumer Phase-1 behavior preserved; canonical `screens/b2c/SOSModal` single mount; EmergencySOS preserved; browser QA 390/430/768/1024/1366 green; Wave 2 physical native confidence remains **NOT RUN**; Pack40DR wait-state preserved; Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_MOBILE_SOS_LEFT_RAIL_REACHABILITY_REMEDIATION_EVIDENCE.md`. |
| Mobile SOS shell Phase 1 + left-rail remediation (combined) | **`VIONA_MOBILE_SOS_SHELL_PHASE_1_CLOSED_GREEN_ON_MASTER`** — source implementation merged+verified on master @ `1eab439` / docs sync PR #394 @ `e141104`; left-rail reachability restored; browser/local responsive gates green; physical native-device confidence **NOT RUN**. Do **not** claim physical iOS/Android PASS, deployment, or production runtime validation. |
| Mobile profile/language contextualization Phase 2 | **MERGED ON MASTER VIA PR #395** — squash `fec4a74` (`2026-07-18T14:26:49Z`); feature OID `7f70b77`; operator phrase `APPROVE_VIONA_MOBILE_PROFILE_LANGUAGE_CONTEXTUALIZATION_PHASE_2`; removes legacy absolute floating ProfileSwitcher / SmartTrioLanguageChip presentation; mounts `VionaShellAccountLanguageActions` in bottom-tab + left-rail chrome (same exact-one gate as SOS); fashion / Local / Travel / Academy keep contextual hosts; disposition **`KEEP_MERGED_PHASE2_UNCHANGED`**; Wave 2 physical native **NOT RUN**; Pack40DR wait-state preserved; Pack40S **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_MOBILE_PROFILE_LANGUAGE_CONTEXTUALIZATION_PHASE_2_EVIDENCE.md`. Do **not** claim physical-native Phase-2 verification or closed-green product readiness beyond merged source. |
| Modern Home Fashion-Tech mobile activation | **Phase A CLOSED GREEN** (PR #397/#398) + **Phase B MOBILE-WEB ACTIVATION VERIFIED ON MASTER** (PR #399/#400) + **Phase C NATIVE ADAPTATION CLOSED GREEN VERIFIED ON MASTER** (PR #401–#403; `TERMINAL_CANONICAL_SYNC_RULE` / `NO_FURTHER_SELF_REFERENTIAL_DOCS_PR_REQUIRED`) — classification `VIONA_MODERN_HOME_NATIVE_ADAPTATION_VERIFIED_ON_MASTER` / closed-green post-merge verify; architecture `SHARED_ADAPTIVE_NATIVE_REUSE`; eligible iOS/Android B2C Home adaptive (native never `desktop`); web Phase-B + desktop Fashion-Tech **preserved**; legacy native fallback **retained**; SOS Phase 1 + PR #395 exact-one hosts **preserved**; Android emulator QA **PASS** (**NOT PHYSICAL DEVICE PASS**). **Phase D1 Android physical confidence:** `BLOCKED_NO_STABLE_PHYSICAL_ANDROID_DEVICE` (PR #404 @ `f5a0706`; emulator rejected). **iOS readiness lineage:** PR #405–#408 @ `db6c878` (retrospective CI verified after #408; preexisting Functions TSC debt separate) → **iOS EAS development build execution:** operator phrase `APPROVE_VIONA_IOS_EAS_DEVELOPMENT_BUILD_PREPARATION_EXECUTION`; source `db6c878`; `ci:expo-readiness`/`ci:release-discipline` PASS; Expo account/project access OK; **`BLOCKED_APPLE_DEVELOPER_ACCOUNT_UNAVAILABLE`** (no Apple teams for authenticated Expo account; no device registration/build started); Phase D2 **NOT AUTHORIZED**. Evidence: `docs/product/VIONA_IOS_EAS_DEVELOPMENT_BUILD_EXECUTION_EVIDENCE.md`. |
| Feature-complete gap audit + critical path | **MERGED ON MASTER** — PR #410 @ `86fb9fb`; operator phrase `APPROVE_VIONA_FEATURE_COMPLETE_GAP_AUDIT_AND_CRITICAL_PATH_PLAN`; scorecard **16/24 (~67%)** — not feature-complete; critical-path audit remains canonical. Evidence: `docs/product/VIONA_FEATURE_COMPLETE_GAP_AUDIT_AND_CRITICAL_PATH_PLAN.md`. |
| FC-P0 Local create client boundary addendum | **MERGED / VERIFIED ON MASTER** — PR #411 @ `1c631dc`; operator phrase `APPROVE_VIONA_FC_P0_LOCAL_REQUEST_CREATE_CLIENT_BOUNDARY_ADDENDUM`; resolves post-#410 `BLOCKED_LOCAL_CREATE_IMPLEMENTATION_BOUNDARY_UNRESOLVED`; locks entry=`LocalUserRequestStatusScreen` in-screen composer, `POST /api/local/requests`, DTO/auth/state machine/duplicate-submit/post-create refresh. Evidence: `docs/product/VIONA_FC_P0_LOCAL_REQUEST_CREATE_CLIENT_IMPLEMENTATION_BOUNDARY_ADDENDUM.md`. |
| FC-P0 Local request create client implementation | **MERGED ON MASTER** — PR #412 @ `028187c`; wires B2C create to `POST /api/local/requests`; post-merge strict review found first-time business-context / raw-UUID UX / submit-guard race blockers — remediated by next row. Evidence: `docs/product/VIONA_FC_P0_LOCAL_REQUEST_CREATE_CLIENT_IMPLEMENTATION_EVIDENCE.md`. |
| FC-P0 Local create business-context + submit-guard remediation | **MERGED ON MASTER** — PR #413 @ `8bc568b`; raw UUID removed + submit-guard before JWT await; **strict review blocked** tourism discover as Local provider source (`BLOCKED_TOURISM_DISCOVER_NOT_VALID_LOCAL_PROVIDER_SOURCE`). Evidence: `docs/product/VIONA_FC_P0_LOCAL_CREATE_BUSINESS_CONTEXT_AND_SUBMIT_GUARD_REMEDIATION_EVIDENCE.md`. |
| FC-P0 Local provider eligibility + Tourism coupling remediation | **MERGED / VERIFIED ON MASTER** — PR #414 @ `19dbc2f`; Path 2 containment; `PROVIDER_SELECTION_UNAVAILABLE`; Tourism discover removed from Local create; UUID removal + submit guard preserved; **FC-P0 still blocked** (no Local eligibility authority). Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_AND_TOURISM_COUPLING_REMEDIATION_EVIDENCE.md`. |
| FC-P0 Local provider eligibility authority boundary + implementation plan | **MERGED ON MASTER** — PR #415 @ `dc5c625`; architecture locked. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_BOUNDARY_AND_IMPLEMENTATION_PLAN.md`. |
| FC-P0 Local provider authority plan lifecycle/write/audit/consistency remediation | **MERGED ON MASTER** — PR #416 squash `@ 435ddcd0f59b6e9295755398a54482788d7948ed` (reviewed head `e83bc6727ad59a3d2d18396ea635ee81a5574eb4`). Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_LIFECYCLE_WRITE_AUDIT_CONSISTENCY_REMEDIATION.md`. |
| FC-P0 Local provider authority audit-model + implementation-readiness remediation | **MERGED ON MASTER** — PR #417 squash `@ d3bd2935b7ff8029eb5e4c96869c70f1bf1a54ac` (reviewed head `c581a78a47cc033bd5fef0e741081b987d552a6f`); strict review **blocked** on lifecycle timestamps / Restrict graph / REGISTERED prior-list / PATCH-by-status. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_AUDIT_MODEL_AND_IMPLEMENTATION_READINESS_REMEDIATION.md`. |
| FC-P0 Local provider authority timestamp / referential / PATCH finalization | **MERGED / VERIFIED ON MASTER** — PR #418 squash `@ 4d0e6cff083f86ef153979446be4785bdbcfb73b`; locks latest `activatedAt`, clear `suspendedAt` on reactivation, immutable `retiredAt`, Restrict retention graph, REGISTERED prior null/`[]`, PATCH-by-status incl. RETIRED→409, executable A1 card. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_PLAN_TIMESTAMP_REFERENTIAL_PATCH_FINALIZATION.md`. |
| FC-P0 Local provider eligibility Pack A1 — schema + domain enforcement | **MERGED / VERIFIED ON MASTER** — PR #419 squash `@ 97d9ee01f6159a7ed7d48ac3b422ddb4359bca60`; structure-only migration (unapplied); domain helpers + create enforcement; A1 tests 1–29; FC-P0 still blocked pending A2+B at merge time. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_SCHEMA_DOMAIN_ENFORCEMENT_IMPLEMENTATION_EVIDENCE.md`. |
| FC-P0 Local provider eligibility Pack A2 — read + ops control | **MERGED ON MASTER** — PR #420 squash `@ ff0fdfa7752eacb265e94f1de75e676d57ae5c62`; GET `/api/local/providers` + ADMIN ops register/PATCH/activate/suspend/retire; append-only audit writes; **strict review blocked** `BLOCKED_LOCAL_PROVIDER_A2_DB_BEHAVIOR_EVIDENCE_INSUFFICIENT` (cases 30–43 reported PASS while DB branch skipped). Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_READ_AND_OPS_CONTROL_IMPLEMENTATION_EVIDENCE.md`. |
| FC-P0 Local provider Pack A2 — deterministic behavioral evidence remediation | **MERGED ON MASTER** — PR #421 squash `@ 0dab3f40053f1601beabe77ced9a6da990f9b954`; production Prisma-default DI + scripts-only deterministic store/txn/clock; cases 30–43 + rollback executed without migration apply. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_A2_DETERMINISTIC_BEHAVIORAL_EVIDENCE_REMEDIATION.md`. |
| FC-P0 Local create failure-code envelope + Pack B recovery discrimination | **MERGED / VERIFIED ON MASTER** — PR #423 squash `@ c7b936595abaf8ffca8d12687833ef47e4be5791`; reviewed head `6be102fbb199835a105ae13c3c95082f9fae8c21`; Local create fail envelope adds allowlisted `code`; recovery only for `provider_not_available` (404) + `service_type_not_supported` (400); post-merge verification green; **no** migrate-apply / deploy / activation / live QA; FC-P0 still blocked pending execution gates; `REQUEST_ONLY_NO_CHARGE`; Pack40S **NOT AUTHORIZED**; Apple/EAS/Phase D2 deferred; Phase C closed green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_CREATE_FAILURE_CODE_ENVELOPE_AND_PACK_B_RECOVERY_DISCRIMINATION.md`. |
| FC-P0 Local provider Pack B — client authority wiring | **MERGED on master @ 5ef5a1d** (PR #422) — create-recovery remediated by PR #423; prior evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_ELIGIBILITY_AUTHORITY_CLIENT_WIRING_IMPLEMENTATION_EVIDENCE.md`. |
| FC-P0 Local provider authority controlled execution planning packet | **MERGED / VERIFIED ON MASTER** — PR #424 squash `@ 50c2c7616184c3b8f0a85bf224bc30a4daf526d6`; reviewed head `c275eb2c7ec015b63cf27270fc6be09a110a7b45`; docs-only E0–E10 planning; post-merge verify green with E1–E10 unauthorized. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_EXECUTION_PLANNING_PACKET.md`. |
| FC-P0 Local provider authority staging execution preflight (E1) | **MERGED ON MASTER** — PR #425 squash `@ 098cb8709a980aa6a9df76a6ca6a1e4fffa9478d`; reviewed head `81004a6f4f1be374f6ee6b0fbd71f4c31b492636`; read-only staging preflight recorded; **strict review blocked** backup/restore freshness (`BLOCKED_E1_BACKUP_RESTORE_EVIDENCE_STALE_OR_INSUFFICIENT`); Pack A1 observed unapplied; Fly v28 / rollback v27 preserved; status later corrected by #426 + operator-mediated capture. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_EXECUTION_PREFLIGHT_RESULT.md`. |
| FC-P0 Local provider authority E1 backup/restore evidence remediation | **MERGED ON MASTER** — PR #426 squash `@ 777c87662174ba3445ee88d293c27e4506dddf3f`; corrects Pack A1 **Git/LF** checksum `3B028C85…` / 3471 B; withdraws unsupported `BACKUP_RESTORE_READINESS_CONFIRMED`; at merge fresh CLI `supabase backups list` blocked (no management token) → then `BLOCKED_E1_EXPLICIT_FC_P0_RISK_ACCEPTANCE_REQUIRED`; risk-acceptance proposal **PROPOSED/NOT GRANTED**; **no** backup create / restore / migrate-apply / deploy / provider mutation / live QA; E2 **not** authorized. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E1_BACKUP_RESTORE_EVIDENCE_REMEDIATION.md`, `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E2_BACKUP_RESTORE_RISK_ACCEPTANCE_PROPOSAL.md`. |
| FC-P0 Local provider authority E1 operator-mediated backup metadata capture | **MERGED / VERIFIED ON MASTER** — PR #427 squash `@ 91b7aaf33384142ee2e7dc48c4681c6b649ec8a9`; reviewed head `9fb3acacc5af34ac73552abbecaf398a74261df5`; operator dashboard evidence confirms staging recovery point **`2026-07-23T02:42:05Z` PHYSICAL COMPLETED**; classification `CURRENT_STAGING_RECOVERY_POINT_CONFIRMED_BY_OPERATOR_DASHBOARD_EVIDENCE`; post-merge verify green with E2 ready for authorization decision but unauthorized at that time. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E1_OPERATOR_MEDIATED_BACKUP_METADATA_CAPTURE_RESULT.md`. |
| FC-P0 Local provider authority E2 staging migration apply (Pack A1) | **MERGED / VERIFIED ON MASTER** — PR #428 squash `@ de59110a6c9a1bfef7fc0d7c68be24513ffe9781`; reviewed head `15eb9438a6102e02726367e0704f3657b397460e`; staging-only Pack A1 apply @ `2026-07-23T10:06:06Z` exit 0; history clean; zero eligibility/audit rows; post-merge verify green with E3 ready for authorization decision but unauthorized at that time. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_MIGRATION_APPLY_RESULT.md`. |
| FC-P0 Local provider authority E3 staging API deploy | **MERGED / VERIFIED ON MASTER** — PR #429 squash `@ 3f724e06612da48eecb8a0931e3255727bc2a204`; one `fly deploy --app viona-api-staging-eu --remote-only` @ `2026-07-23T10:45:07Z`–`10:48:14Z` exit 0 → Fly **v29** / image `deployment-01KY798FWDYE8YM0ZD4QW98JP0` / digest `sha256:b4c42e5c…`; pre-deploy rollback target exact **v28**; `/health` 200; unauth Local routes **401**; Pack A1 applied/clean; rows 0/0; secondary canonical §9 `READY_FOR_CONTROLLED_PROVIDER_REGISTRATION` (did **not** authorize E4/E6). Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_API_DEPLOY_RESULT.md`. |
| FC-P0 Local provider authority E6 staging provider registration/configuration | **MERGED / VERIFIED ON MASTER** — PR #431 squash `@ f59e723fd18ee4854c0e92739b81d085112802e0`; reviewed head `4537ef38d925e365acd2cc30f80546d99b1ff6fe`; one `POST /api/local/ops/providers` @ `2026-07-23T16:01:33Z` → HTTP **201**; DRAFT / visibility false / `GENERIC_REQUEST`; audit REGISTERED only; eligibility/audit 1/1; conclusion `CONTROLLED_PROVIDER_REGISTERED_AND_CONFIGURED_IN_DRAFT_WITH_VISIBILITY_DISABLED`. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_REGISTRATION_AND_CONFIGURATION_RESULT.md`. |
| FC-P0 Local provider authority E7 staging provider activation | **MERGED / VERIFIED ON MASTER** — PR #432 squash `@ b2e84f88be71ab50e4494de1832a86cd92569679`; reviewed head `8eca263d22f431642eda839f68f6534fef650390`; one `POST .../activate` @ `2026-07-23T16:37:09Z` → HTTP **409**; primary `BLOCKED_E7_SEPARATE_VISIBILITY_ACTION_NOT_AUTHORIZED`; provider remained DRAFT / visibility false at that time; post-merge verify green with provider unchanged. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_ACTIVATION_RESULT.md`. |
| FC-P0 Local provider authority staging public B2C visibility enablement | **MERGED / VERIFIED ON MASTER** — PR #433 squash `@ d39a1a11faddabe14c205d485f78f9cb68bfc337`; reviewed head `da5d2502094b51697f9237939adf0b1f9a45f238`; one `PATCH .../providers/:businessId` `{publicB2cVisible:true}` @ `2026-07-23T17:10:15Z` → HTTP **200**; conclusion `CONTROLLED_PROVIDER_PUBLIC_B2C_VISIBILITY_ENABLED_WHILE_REMAINING_DRAFT`; audit REGISTERED→CONFIG_UPDATED; rows 1/2; post-merge verify green (activation retry was unauthorized **at that time**). Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_PUBLIC_B2C_VISIBILITY_ENABLEMENT_RESULT.md`. |
| FC-P0 Local provider authority staging provider activation retry | **MERGED / VERIFIED ON MASTER** — PR #434 squash `@ 1110c21b8b83fbd2dc2f83846ea795a8027122c3`; reviewed head `067b717a8f80dd508281a07d26531bcbe0231701`; one `POST .../activate` empty body @ `2026-07-23T18:50:15Z` → HTTP **200**; conclusion `CONTROLLED_STAGING_PROVIDER_ACTIVATED_AFTER_PUBLIC_B2C_VISIBILITY_ENABLEMENT`; ACTIVE / visibility true / `GENERIC_REQUEST`; audit REGISTERED→CONFIG_UPDATED→ACTIVATED; rows 1/3; post-merge verify green; E8–E10 remained unauthorized. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_STAGING_PROVIDER_ACTIVATION_RETRY_RESULT.md`. |
| FC-P0 Local provider authority post-activation next-stage decision packet | **MERGED / VERIFIED ON MASTER** — PR #435 squash `@ a98a3222a0a5a637088693d8fe147861210070b1`; docs-only; recommendation `RECOMMEND_E8_FOR_SEPARATE_OPERATOR_AUTHORIZATION_DECISION` (**not** an E8 grant); E8–E10 remained unauthorized at merge. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_POST_ACTIVATION_NEXT_STAGE_DECISION_PACKET.md`. |
| FC-P0 Local provider authority E8 staging client deployment readiness + prerequisite packet | **MERGED ON MASTER** — PR #436 squash `@ 7f001c6fd403be795c812e247856d23af08a148f` (reviewed head `b249166…`); docs-only; case `E8_CASE_A_DOCS_ONLY_RECOMMENDED`; Case B blockers preserved; E8 phrase remains NOT GRANTED. Post-merge validation later **closed** after PR #438 dist-exclusion remediation. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_STAGING_CLIENT_DEPLOYMENT_READINESS_AND_PREREQUISITE_PACKET.md`. |
| PR #436 TypeScript RangeError no-product-change tooling triage | **MERGED / VERIFIED ON MASTER** — PR #437 squash `@ 48da52e4e2619a1b2e5a268e2841a858be1f4fae`; primary root cause `BLOCKED_TYPESCRIPT_RANGEERROR_INPUT_SCOPE_RECURSION`. Evidence: `docs/product/VIONA_PR436_TYPESCRIPT_RANGEERROR_NO_PRODUCT_CHANGE_TOOLING_TRIAGE_PACKET.md`. |
| PR #436 TypeScript RangeError dist exclusion (no-product-change) | **MERGED / VERIFIED ON MASTER** — PR #438 squash `@ c9292d74015edd8e10cc1db97f560c44bcf50e76` (reviewed head `5421ee7…`); root `tsconfig.json` exclude adds `dist/**`; `GENERATED_DIST_EXCLUDED_FROM_ROOT_TYPESCRIPT_INPUT_SCOPE`; post-merge verify green with required validation; `PR436_POST_MERGE_VALIDATION_CLOSED_AFTER_DIST_EXCLUSION_REMEDIATION`; E8 Case A preserved; Case B blockers preserved; E8–E10 unauthorized. Evidence: `docs/product/VIONA_PR436_TYPESCRIPT_RANGEERROR_DIST_EXCLUSION_NO_PRODUCT_CHANGE_IMPLEMENTATION_RESULT.md`. |
| FC-P0 Local provider authority E8 Case B prerequisite remediation decision packet | **MERGED / VERIFIED ON MASTER** — PR #439 squash `@ 8ca1dbb9339ee6112490f22c7afc42d72dfe68d5` (reviewed head `7b960fc…`); docs-only; case `E8_CASE_B_PREREQUISITE_REMEDIATION_SEQUENCE_READY_FOR_SEPARATE_AUTHORIZATION`; four Case B blockers preserved; B0–B7 defined and unauthorized; E8 deploy phrase remains NOT GRANTED; post-merge verify green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_PREREQUISITE_REMEDIATION_DECISION_PACKET.md`. |
| FC-P0 Local provider authority E8 Case B B0 target and hosting decision | **MERGED / VERIFIED ON MASTER** — PR #440 squash `@ 331d610c25c795f16c1be5c4cf1fe0d56b4ffaa1` (reviewed head `409f638…`); docs-only; decision `B0_DEDICATED_VERCEL_STAGING_PROJECT_RECOMMENDED_FOR_SEPARATE_B1_AUTHORIZATION`; no project created/bound; four Case B blockers preserved; B1 unauthorized at merge; post-merge verify green. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_B0_TARGET_AND_HOSTING_DECISION.md`. |
| FC-P0 Local provider authority E8 Case B B1A Vercel staging project provisioning | **MERGED / VERIFIED ON MASTER — BLOCKED RESULT** — PR #441 squash `@ 5f658cd08842c5be58de5f29f673cb285bb002f2`; case `BLOCKED_B1A_VERCEL_CLI_NOT_AVAILABLE`; project create 0; link 0; deploy 0; Git connect 0; four Case B blockers preserved; B1A retry unauthorized; B1B–B7 unauthorized; E8–E10 unauthorized. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_B1A_VERCEL_STAGING_PROJECT_PROVISIONING_RESULT.md`. |
| FC-P0 Local provider authority E8 Case B Vercel CLI tooling-preparation decision | **MERGED / VERIFIED ON MASTER** — PR #442 squash `@ d4fcce082b284884caacab2fd550ead558a7c716` (reviewed head `999ac17…`); decision `VERCEL_CLI_PINNED_GLOBAL_INSTALLATION_RECOMMENDED_FOR_SEPARATE_AUTHORIZATION` (pin `vercel@56.5.0`); install/login phrases remained PROPOSED at merge; B1A CLI blocker held until separate install lane; four Case B blockers preserved. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_VERCEL_CLI_TOOLING_PREPARATION_DECISION.md`. |
| FC-P0 Local provider authority E8 Case B Vercel CLI pinned tooling installation | **MERGED / VERIFIED ON MASTER** — PR #443 squash `@ c83a7500bcf45a08bc9e9dc6968ac99ce7f929df` (reviewed head `30326eb…`); `vercel@56.5.0` pinned global install verified; `VERCEL_CLI_PINNED_GLOBAL_INSTALLATION_VERIFIED`; `B1A_CLI_PREREQUISITE_REMEDIATED_WITH_LOGIN_AND_RETRY_UNAUTHORIZED`; four Case B blockers preserved. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_VERCEL_CLI_PINNED_INSTALLATION_RESULT.md`. |
| FC-P0 Local provider authority E8 Case B Vercel CLI operator interactive login | **MERGED / VERIFIED ON MASTER** — PR #444 squash `@ 20b6bce37810c51bd54fe5a4226571bb33a1528b` (reviewed head `53d776e…`); `VERCEL_CLI_OPERATOR_INTERACTIVE_LOGIN_VERIFIED`; `B1A_AUTHENTICATION_PREREQUISITE_REMEDIATED_WITH_PROJECT_PROVISIONING_RETRY_UNAUTHORIZED`; four Case B blockers preserved. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_VERCEL_CLI_OPERATOR_INTERACTIVE_LOGIN_RESULT.md`. |
| FC-P0 Local provider authority E8 Case B B1A staging project provisioning retry | **MERGED — FACTUAL RESULT GREEN AND RETAINED ON MASTER** — PR #445 squash `@ 20711265cd25202a4426be740697b5a9d67c113c` (reviewed head `899b5ce…`; parent `20b6bce…`; mergedAt `2026-07-24T18:22:23Z`); created `viona-web-staging-eu` under `ket-noi-global`; local link verified; deployments/env = 0; Git/domains/aliases **NOT RECONFIRMED BY CURRENT CLI CONTRACT**; transient link side effect remediated (historical OIDC/gitignore mutation counts preserved; residual = 0); classifications include `DEDICATED_VERCEL_STAGING_PROJECT_PROVISIONED_WITH_ZERO_DEPLOYMENTS`, `B1A_LOCAL_PROJECT_LINK_VERIFIED_WITH_GIT_BINDING_UNAUTHORIZED`, `B1A_VERCEL_LINK_TRANSIENT_LOCAL_ENV_AND_GITIGNORE_MUTATION_REMEDIATED_WITH_NO_OIDC_SECRET_OR_TRACKED_DIFF_REMAINING`; **historical pre-merge fixed-head merge authorization NOT PROVEN / REMAINS PERMANENTLY UNRESOLVED**; first permanent non-retroactive exception accepted (`ACKNOWLEDGE_AND_ACCEPT_…_PR445_…_AS_NON_RETROACTIVE_GOVERNANCE_EXCEPTION`) — `NO_RETROACTIVE_AUTHORIZATION_CLAIMED`; four Case B blockers preserved; B1B–B7 unauthorized. Evidence: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_B1A_VERCEL_STAGING_PROJECT_PROVISIONING_RETRY_RESULT.md`; governance: `docs/product/VIONA_FC_P0_LOCAL_PROVIDER_AUTHORITY_E8_CASE_B_PR445_MERGE_AUTHORIZATION_PROVENANCE_GAP_NON_RETROACTIVE_EXCEPTION_DECISION.md`. |
...
Explicitly logged here per operator instruction so no item is lost between sessions. Neither item
below is authorized by this log entry — each requires its own future, separately-authorized pack.

1. **Pack36B: Merchant Admin UI (Frontend AI Persona & Webhook Secret configuration)** — Deferred
   to prioritize backend E2E (Pack35 webhook routing -> Pack36A staging QA -> Pack37 dispatcher
   realization). No merchant-facing UI exists today to configure `MerchantProfile.aiPersona`,
   rotate a `VionaMerchantWebhookChannel.signingSecretHash`, or manage `toolScope` — every write to
   these fields so far has gone through a standalone script (`scripts/provision-staging-webhook-test-channel.ts`)
   or a future admin-only backend endpoint (Pack34 §9), never a merchant-operated screen. Candidate
   next planning packet once the backend chain above is fully E2E-verified end-to-end.
2. **`TourismBooking` Prisma migration shadow-DB replay issue** — Deferred tech debt from Pack34 /
   Pack34.5. `20250430120000_tourism_net_gross_platform_fields` is correctly marked `APPLIED` in the
   live database's own migration ledger (re-verified during Pack34.5), but `prisma migrate dev`'s
   shadow-DB replay of the full migration history still fails against this specific migration file —
   its SQL does not match the live `TourismBooking` table's actual shape, a pre-existing drift from
   before Pack34. Live data and the live schema are unaffected; only `prisma migrate dev`'s own
   shadow-DB workflow is impacted. Left for a future, dedicated, standalone fix — never touched by
   Pack34, Pack34.5, Pack35, Pack36A, or Pack37, none of which needed a shadow-DB replay.

(The 2 findings previously logged here as items 3–4 — the double-LLM-classification-call
performance/safety finding and the Pack37 test #19 real-OpenAI-call financial leak, both surfaced
during Pack38's session — were addressed by Pack39 implementation (see the Pack39 implementation
table row above and `docs/product/VIONA_PACK39_TECH_DEBT_ERADICATION_PLAN.md`); removed from this
generic backlog list during Pack39 planning and remain closed after Pack39 implementation.)
...
| Target (non-secret label) | `viona-staging-eu` / `staging-redacted` (staging only) |
...
| Backing database | Same real Supabase project as staging (ref `staging-redacted`) — pre-existing local `.env`, not modified this session |
...
| Supabase project ref | `CONFIRMED — staging-redacted` |
...
| Target | `viona-staging-eu` / `staging-redacted` |
...
```text
APPROVED Pack15C execution approval phrase for the existing VIONA Request migration targeting staging Supabase project `viona-staging-eu` / `staging-redacted`. I confirm DB apply may be planned in a separate execution-only DB apply pack, but must not be performed in this intake pack.
```
...
```text
I, Nong Si Buong, give explicit Pack15C operator GO for the staged DB apply readiness path targeting Supabase project `viona-staging-eu` / `staging-redacted`. I understand DB apply is still not performed by this phrase alone and remains blocked until ChatGPT GO/NO-GO review and a separate execution-only DB apply pack are completed.
```
...
| 2 | DB provider / host | `CONFIRMED — Supabase Postgres project viona-staging-eu / ref staging-redacted` |
...
| 13 | Operator go/no-go | **`PROVIDED — Nong Si Buong phrase recorded verbatim (PR #119); targets viona-staging-eu / staging-redacted; operator GO not invented; not DB apply; not execution-only DB apply pack authorization`** |
| 14 | Separate execution approval phrase | **`PROVIDED — human/operator phrase recorded verbatim (PR #117); targets viona-staging-eu / staging-redacted; phrase not invented; not operator GO; not execution-only DB apply pack authorization`** |
...
| `pack15SupabaseTargetProjectRef` | `staging-redacted` |
...
| `pack15OperatorGoTarget` | `viona-staging-eu / staging-redacted` |
...
| `pack15ExecutionApprovalPhraseTarget` | `viona-staging-eu / staging-redacted` |
