...
| Pack34 B2B Merchant Gateway & AI White-Labeling (planning) | **CLOSED / GREEN** — PR #(pending — see branch `docs/pack34-b2b-merchant-gateway-planning`) — docs-only design for `MerchantProfile` multi-tenant isolation (closes the `VionaRequest.tenantId` free-text-with-zero-enforcement gap found this session), per-merchant AI Persona storage, an additive `'merchant_read_only_query'` Tool Registry category (2 illustrative read-only tools, no write-capable tool designed), and how a future write tool would reuse the existing, unmodified Pack31 escrow hold/settle with the merchant's own `ownerUserId`/`Wallet` — no code, no schema, explicitly deferred write-tool/activation-workflow/pricing-formula design; see `docs/product/VIONA_PACK34_B2B_MERCHANT_GATEWAY_PLAN.md` |
