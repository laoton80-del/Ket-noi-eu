...
| Pack40 multi-tenant VionaRequest access enforcement (planning) | **REFINED v3 / Pack40A BLOCKED behind Pack40P** — PR #340–#343 merged @ `1af07e7`. Inventory: 10 staging requests, 5 unresolved, **`canonicalConsumerProvenanceConfirmed: false`**. **Pack40P planning** (`docs/product/VIONA_PACK40P_REQUEST_PROVENANCE_MODEL_PLAN.md`): additive `VionaRequestScopeKind`, `scopeKind` DB default `legacyUnresolved`, optional `merchantProfileId` FK; **rollout order P1→P3→P2→P2D→P4→P5**; **P1 deployment lock** (no app deploy until migration applied per environment); new phrase **`APPROVE_PACK40P2D_STAGING_CREATE_PATH_PROVENANCE_DEPLOY`**; Pack19 `scopeKind` reflects **creation path** not actor profile (dual-role safe). **No schema/product code authorized by planning.** Pack40A blocked until Pack40P definition-of-ready. |
